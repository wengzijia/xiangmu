<template>
  <div class="upload-file">
    ...
    <div class="el-upload__tip" v-if="showTip&&!showFile">
      请上传<template v-if="fileType">格式为 <b style="color: #333">{{ fileType.join("/") }}</b></template><template v-if="fileSize"> 大小不超过 <b style="color: #333">{{ fileSize }}MB</b> </template>
      的文件
    </div>
    <!-- 文件列表 -->
    <transition-group  class="upload-file-list el-upload-list el-upload-list--text" name="el-fade-in-linear" tag="ul" style="display: inline-block;">
      <li :key="file.uid" class="el-upload-list__item ele-upload-list__item-content" v-for="(file, index) in fileList">
		<el-link :href="senseUrl(fileUrl(file.fileAddress||file.url))" :underline="false" target="_blank">
			<span class="el-icon-document text-overflow" style="max-width:500px;color: #409EFF;margin-right:10px;"> {{getFileName(file.name) }} </span>
		</el-link>
