      ...
      <div>
        <el-form :model="listData" label-width="110px" class="approval">
          ...
          <el-form-item label=" 纹理名称:">
            <div class="flex_r">
              {{listData.before.embossTextureName||'-'}}
            </div>
            <div :class="{'flex_l':true,'red_text':materialColor.embossTextureName}" v-show="NotShow">
              {{listData.after.embossTextureName||'-'}}
            </div>
          </el-form-item>
          <el-form-item label="纹理编码:">
            <div class="flex_r">
              {{listData.before.embossTextureNo||'-'}}
            </div>
            <div :class="{'flex_l':true,'red_text':materialColor.embossTextureNo}" v-show="NotShow">
              {{listData.after.embossTextureNo||'-'}}
            </div>
          </el-form-item>
...
getEmbossTextureId(drawerData.value)
...
function getEmbossTextureId(val) {
let drdata=val
recordId.value=val.id
if(drdata){
  embossTextureId(drdata.id).then((res) => {
    if (res.code === 200) {
