<template>
  <div class="drawer">
    <el-form :model="form" ref="gildingForm" :rules="rules" label-width="120px">
      <el-form-item label="颜色示例图片">
      ...
      <el-form-item label="烫金颜色名称" prop="bronzeColorName">
        <div v-show="!disabled" style="width:100%">
          <el-input v-model.trim="form.bronzeColorName" maxlength="50" show-word-limit placeholder="请输入烫金颜色名称"/>
        </div>
        <div v-show="disabled" style="width:100%">
          <div>{{drawerData.bronzeColorName||'-'}}</div>
        </div>
      </el-form-item>
      <el-form-item label="烫金颜色代码" prop="bronzeColorCode">
        <div v-show="!disabled" style="width:100%">
          <el-input v-model.trim="form.bronzeColorCode" maxlength="50" show-word-limit placeholder="请输入烫金颜色代码"/>
        </div>
        <div v-show="disabled" style="width:100%">
          <div>{{drawerData.bronzeColorCode||'-'}}</div>
        </div>
      </el-form-item>
      ...
      <el-form-item>
        <div v-show="!disabled">
          ...
          <el-button type="primary" v-if="drawerType=='add'||drawerType=='copy'" @click="confirmChange(gildingForm)" v-hasPermi="['maintain:add']">提交审批</el-button>
          <el-button type="primary" v-else @click="confirmChange(gildingForm)" v-hasPermi="['maintain:change']">提交审批</el-button>
        </div>
      </el-form-item>
    </el-form>
  </div>
...
import { addBronzeColor, changeBronzeColor, } from '@/api/basicDatabase'
...
const rules = reactive({
  bronzeColorName: [{ required: true, message: '请输入烫金颜色名称', trigger: 'blur' },],
  bronzeColorCode: [{ required: true, message: '请输入烫金颜色代码', trigger: 'blur' },],
})
...
async function editTable() {
  ...
  changeBronzeColor(data).then(res => {
    if (res.code === 200) {
      ElMessage({
        type: 'success',
        message: '修改成功',
...
async function addTable() {
  let data = {
    bronzeColorCode: form.value.bronzeColorCode || undefined,
    bronzeColorName: form.value.bronzeColorName || undefined,
