      ...
      <div class="top-bar subOrder">
        <el-form inline :model="formData" class="condition">
          ...
          <el-form-item label="订单完成时间:">
            <dateTimePicker ref="dateRef1" :dateType="'datetimerange'" v-model:startTime="formData.orderFinishStartTime"
              v-model:endTime="formData.orderFinishEndTime" :isThreeMonths="false"></dateTimePicker>
            <!-- <el-date-picker style="width: 360px" v-model="endTime" type="datetimerange" unlink-panels range-separator="→"
              start-placeholder="开始日期" end-placeholder="结束日期" 
              @change="timeFormat(endTime, '2')" /> -->
          </el-form-item>
          <!-- 订单状态 -->
          ...
          <el-form-item label="订单信息:">
            <el-input style="width: 252px" clearable v-model="formData.orderInfo" placeholder="请输入订单编号/合同编号" />
          </el-form-item>
        </el-form>
        <div class="btnBox">
    ...
    <el-card style="margin: 10px" class="artwork-workbench">
      ...
      <div class="outworkListBox">
        <el-table :data="tabList" style="width: 100%" scrollbar-always-on>
          ...
          <!-- <el-table-column label="成本单价" width="200">
  						<template #default="scope">
  							{{
          ...
          <el-table-column label="成本总价" width="120">
            <template #default="scope">
          ...
          <el-table-column width="160">
            <template #header>
              ...
              <el-tooltip class="box-item" effect="dark" placement="bottom-end">
                <template #content>方案设计订单展示设计合同，<br />其他订单展示大货合同
                </template>
                <el-icon class="icons">
                  <WarningFilled />
                </el-icon>
 ...
 */
const reset = () => {
  // startTime.value = [formatDate(new Date().getTime() - 3600 * 1000 * 24 * 90), formatDate(new Date().getTime() + 3600 * 1000 * 24 * 1)]
  //开始时间
  // endTime.value = ''
  dateRef.value.initTime(1);
  dateRef1.value.initTime();
  ...
  formData.value = {
    ...
    // orderGenerationStartTime: startTime.value[0], //订单生成时间开始时间
    // orderGenerationEndTime: startTime.value[1], //订单生成时间结束时间
    orderFinishStartTime: "", //订单完成时间开始时间
    orderFinishEndTime: "", //订单完成时间结束时间
    customerInfo: "", //客户姓名,客户编号
    // settlementMethod: '', //结算方式
...
onMounted(() => {
  ...
  watch(
    ...
    (val, oval) => {
      ...
      //   dateRef.value.initTime(1)
      //   dateRef1.value.initTime()
      //   formData.value.procType = '' //订单类型
      //   formData.value.procStatus = '' //流程状态
      ...
      //   // formData.value.settlementMethod = '' //结算方式
