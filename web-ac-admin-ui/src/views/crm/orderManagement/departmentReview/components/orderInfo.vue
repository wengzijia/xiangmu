<template>
  <div>
    <el-form v-if="drawerFormData.contractType == '1'" :model="formData" label-width="160px">
      <div>合同</div>
      <el-form-item label="设计服务合同:">
        <div>
          <span style="word-break: break-all;">{{formData.designServiceContractName}}</span>
    ...
    <el-form v-if="drawerFormData.contractType == '4'" :model="formData" label-width="160px">
      ...
      <el-form-item label="合同/采购单:">
        <div>
          <span style="margin-right: 10px;word-break: break-all;">{{ formData.bulkContractName || '-' }}</span>
    ...
    <div class="contract-history" style="margin:10px" v-if="drawerFormData.contractType == '1'&&auditHistory?.length">
      ...
      <div v-for="item in auditHistory" :key="item.id" class="contract-history-box">
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">驳回类型:</div>
          <div style="flex: 1;"><DictTag :value="item?.contractRejectType" :options="crm_ContractRejectionType" /></div>
        </div>
        <div style="display: flex;margin-top: 6px;">
        ...
        <div style="display: flex;margin-top: 6px;">
          ...
          <div style="flex: 1;">{{ item?.designServiceContractRejectName}}
    ...
    <div class="contract-history" style="margin:10px" v-if="drawerFormData.contractType == '2'&&auditHistory?.length">
      ...
      <div v-for="item in auditHistory" :key="item.id" class="contract-history-box">
        ...
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">驳回人:</div>
          <div style="flex: 1;">{{ item?.contractDeptAuditPersonName||item?.contractAuditPersonName}}</div>
        </div>
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">驳回原因:</div>
          <div style="flex: 1;">{{ item?.contractAuditRejectReason||item?.contractAuditRejectReason}}</div>
        </div>
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">驳回时间:</div>
          <div style="flex: 1;">{{ parseDate(item?.contractDeptAuditTime||item?.contractAuditTime)}}</div>
        </div>
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">合同:</div>
          <div style="flex: 1;">{{ item?.supplierContractName}}
    ...
    <div class="contract-history" style="margin:10px" v-if="drawerFormData.contractType == '3'&&auditHistory?.length">
      ...
      <div v-for="item in auditHistory" :key="item.id" class="contract-history-box">
        ...
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">驳回人:</div>
          <div style="flex: 1;">{{ item?.contractDeptAuditPersonName||item?.supplierContractAuditerName}}</div>
        </div>
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">驳回原因:</div>
          <div style="flex: 1;">{{ item?.contractAuditRejectReason||item?.supplierContractAuditRejectReason}}</div>
        </div>
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">驳回时间:</div>
          <div style="flex: 1;">{{ parseDate(item?.contractDeptAuditTime||item?.supplierContractAuditTime)}}</div>
        </div>
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">合同:</div>
          <div style="flex: 1;">{{ item?.supplierContractName}}
    ...
    <div class="contract-history" style="margin:10px" v-if="drawerFormData.contractType == '4'&&auditHistory?.length">
      ...
      <div v-for="item in auditHistory" :key="item.id" class="contract-history-box">
        ...
        <div style="display: flex;margin-top: 6px;">
          <div class="history-label">合同:</div>
          <div style="flex: 1;">{{ item?.bulkContractRejectName}}
...
import { getDesignServiceContractAuditTotal,getProofContractAuditTotal,getSupplierContractAuditTotal,getBulkContractAuditTotal } from "@/api/crm/finance/history"
...
function getDesing() {
  ...
  getDesignServiceContractAuditTotal(drawerFormData.value.procId).then(res=>{
    if (res.code==200) {
...
function supplierProof() {
  ...
  getProofContractAuditTotal(drawerFormData.value.procId).then(res=>{
    if (res.code==200) {
      auditHistory.value = res.data
    }
  })
...
function getBulk(params) {
  ...
  getBulkContractAuditTotal(drawerFormData.value.procId).then(res=>{
    if (res.code==200) {
      auditHistory.value = res.data
    }
  })
...
function supplierBulk(){
  ...
  getSupplierContractAuditTotal(drawerFormData.value.bulkQuoteId).then(res=>{
    if (res.code==200) {
      auditHistory.value = res.data
    }
  })
