    ...
    <div>
      <el-card style="margin: 10px">
        ...
        <el-form :inline="true" :model="formData" label-width="120px" ref="formRef">
          ...
          <el-form-item label="订单编号:" prop="businessKeyInfo">
            <el-input v-model.trim="formData.businessKeyInfo" placeholder="请输入订单编号" clearable style="width:252px;" />
          </el-form-item>
          <el-form-item label="合同/核价单申请时间:" prop="apply" label-width="160">
            <template #label>
              <span style="font-weight: 700;">合同/核价单申请时间:</span>
            </template>
          ...
          <el-form-item label="合同/核价单审核时间:" prop="examine" label-width="160">
            <template #label>
              <span style="font-weight: 700;">合同/核价单审核时间:</span>
            </template>
          ...
          <el-form-item label="审核状态:" prop="contractDeptAuditStatus">
            <el-select v-model="formData.contractDeptAuditStatus" placeholder="所有" clearable style="width:252px;">
      ...
      <el-card style="margin: 10px">
        ...
        <el-table :data="tableData" style="width: 100%;height: 540px;" row-key="id" v-loading="loading" ref="multipleTable">
          ...
          <el-table-column prop="contractDeptAuditStatus" label="审核状态" width="100">
            <template #default="scope">
              <DictTag :value="scope.row.contractDeptAuditStatus" :options="crm_contract_examine_status" />
            </template>
          </el-table-column>
          <!-- <el-table-column prop="contractAuditPersonName" label="审核人" >
            <template #default="scope">{{ scope.row.contractDeptAuditPersonName||'-' }}</template>
          </el-table-column> -->
          ...
          <el-table-column prop="auditContractTime" label="审核时间" width="160">
            <template #default="scope">{{ parseTime(scope.row.contractDeptAuditTime)||'-' }}</template>
          </el-table-column>
          <el-table-column label="操作" width="120" fixed="right">
            <template #default="scope">
              <div>
                <el-link :underline="false" type="primary" @click="showDrawer(scope.row,'show')" style="margin-right: 10px;">查看</el-link>
                <el-link v-if="scope.row.contractDeptAuditStatus=='1'" :underline="false" type="primary" @click="showDrawer(scope.row,'edit')">审核</el-link>
              </div>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-drawer v-model="drawer" destroy-on-close :title="drawerType=='edit'?'审核':'查看'" :size="600" direction="rtl">
        ...
        <el-form :model="drawerFormData" label-width="150px" ref="dialogForm">
          ...
          <el-form-item label="审核结果:">
            ...
            <div v-else>
              <DictTag v-if="drawerFormData?.contractDeptAuditStatus" :value="drawerFormData.contractDeptAuditStatus" :options="crm_contract_examine_status" />
              <span v-else>-</span>
            </div>
          </el-form-item>
          ...
          <el-form-item v-show="drawerType!='edit'&&drawerFormData.contractDeptAuditStatus=='3'" label="审核不通过原因:">
            <template #label>
              <span style="color:red;margin-right: 4px;">* </span>
              <span>审核不通过原因:</span>
            </template>
            <div>
              <span style="word-break:break-all;">{{ drawerFormData.designServiceContractRejectReason||drawerFormData.contractAuditRejectReason ||drawerFormData.supplierDeptContractAuditRejectReason||drawerFormData.bulkContractRejectReason||'-'}}</span>
            </div>
          </el-form-item>
        </el-form>
        <div>
...
import { getDeptAuditList, bulkDeptContractAudit, designDeptContractAudit, proofDeptContractAudit,supplierDeptContractAudit,assigneeType,exportContractAuditList,getAuditer } from "@/api/crm/finance/contract";
...
const dateRef1 = ref(null)
  ...
  const data = reactive({
    formData: {
      ...
      contractDeptAuditStatus:'1',//审核状态
...
if (crm_acColumn.value.length>0) {
  ...
  function getList() {
    ...
    params.apply = params.examine = undefined
    getDeptAuditList(params).then(res => {
      if (res.code == 200) {
      ...
      .then(() => {
        ...
        new Promise((resolve, reject) => {
          ...
          switch (drawerFormData.value.contractType) {
            case '1':
              resolve(designDeptContractAudit(obj))
              break;
            case '2':
              resolve(proofDeptContractAudit(obj))
              break;
            case '3':
              resolve(supplierDeptContractAudit(obj))
              break;
            case '4':
              resolve(bulkDeptContractAudit(obj))
              break;
