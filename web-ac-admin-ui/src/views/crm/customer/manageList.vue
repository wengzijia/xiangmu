  ...
  <div>
    <list-layout>
      ...
      <template #table-header>
        <div  class="tool-bar">管户信息列表</div>
      </template>
      <template #table-content='scoped'>
        <el-table   :height='scoped.tableHeight' :data="tableData" style="width: 100%" v-loading="loading" row-key="id">
        ...
        <!-- <el-table-column label="操作" fixed="right" width="180">
          <template #default="scope">
            ...
            <el-link v-if="!scope.row.auditing&&scope.row.accountApplicationStatus>2" :underline="false" type="primary" @click="modifyCustomerDetail(scope.row.id)" style="margin-left: 8px">修改信息</el-link>
          </template>
        </el-table-column> -->
      </el-table>
      </template>
      <template #pagination> 
        <pagination style="margin-top: 0;height: auto;" background layout="total, sizes, prev, pager, next, jumper" v-model:page="formData.current"
