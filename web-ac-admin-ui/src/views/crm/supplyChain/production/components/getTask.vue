<template>
  <div class="lookDetail">
    <el-drawer v-model="isTask" direction="rtl" size="600px" title="领取大货任务" @close="cancelClick">
      <template #default>
        <div>
          <el-form :model="formData" :rules="rules" label-width="120px" ref="supplyRef">
            ...
            <el-form-item label="供应链专员：" prop="supplyChainSpecialistId">
              ...
              <el-select v-model="formData.supplyChainSpecialistId" class="m-2" style="width: 440px" :disabled="deptDisabled"
            ...
            <el-form-item label="选择品类：" v-show="taskData.type == 2" prop="categoryList">
              <el-table ref="multipleTableRef" :data="categoryList" style="width: 100%"
                ...
                <el-table-column property="bulkProdQuantity" label="数量" width="120" />
                <el-table-column label="待生产数量" width="120">
                  <template #default="scope">{{
                    scope.row.bulkProdQuantity -
                    scope.row.bulkProdReceiveQuantity
                  }}</template>
                </el-table-column>
              </el-table>
            </el-form-item>
          ...
          <el-drawer v-model="innerDrawer" title="选择订单" :append-to-body="true" size="580px" @close="innerCancel">
            <div class="mt-4">
              <el-input v-model="requestParm.keyword" placeholder="请输入订单编号/客户名称" class="input-select" clearable>
                <template #prepend>
                  <el-select v-model="requestParm.procType" class="m-2" style="width: 115px" placeholder="所有类型" clearable >
...
<script setup>
import { bulkReceivesTask } from '@/api/orderManagement/proofing'
...
const supplyRef = ref()
const emits = defineEmits(['bulkQuoteProcFn'])
...
const handleSelectionChange = (list) => {
  ...
  list.map((item) => {
    let obj = {
      ...
      bulkProdQuantity:
        item.bulkProdQuantity - item.bulkProdReceiveQuantity, //打样数量
    }
...
//  供应链专员领取任务接口
const bulkReceivesTaskFn = () => {
  formData.value.supplyChainSpecialistName = bulkSupplyList.value.find((e) => e.userId == formData.value.supplyChainSpecialistId)?.nickName
  const loading = ElLoading.service({
    lock: true,
    text: '加载中',
    background: 'rgba(0, 0, 0, 0.7)',
  })
  bulkReceivesTask(formData.value).then((res) => {
...
const projectDesignOrderProcFn = (id) => {
  projectDesignOrderProc(id).then((res) => {
    if (res.code == 200) {
      ...
      // console.log('方案设计订单详情res444----', res.data)
    }
  })
}
// 获得大货订单详情
...
const bulkDesignProdOrderProcFn = (id) => {
  bulkDesignProdOrderProc(id).then((res) => {
    // console.log('大货设计生产订单详情res444----', res)
    categoryList.value = res.data.categoryList
  })
}
const initData = () => {
