      ...
      <div class="add_new_user_info_box">
        <div class="add_new_info_container">
                  ...
                  <el-form-item class="info_item_content_box" label="姓名" prop="userName">
                    <div class="info_item">
                      <div class="info_item_content">
                        <div class="info_item_input_box"><!-- :disabled="!!userInfoData.id" -->
          ...
          <div class="user_role_box">
            ...
            <div class="addOrg" v-if="isEdit">
              <span class="submitIcon addIcon" @click="handleAddOrg">
            ...
            <div class="user_roles_wrap">
              <div class="user_roles_list">
                <div class="user_roles_box" v-for="(par, pIndex) in orgList">
                  <div class="leftBox">
                    <div class="user_roles_box_top">
                      ...
                      <div class="roles_content">
                        <el-select v-model="par.orgValue" class="m-2" :disabled="manufacturerBossFlag && !isEdit"
                          placeholder="请选择组织" @change="orgChange($event, par)">
                      ...
                      <div v-for="(industrialPark, index) in par.industrialParkListArr" :key="industrialPark.id"
                        ...
                        <div class="role_blank_left">
                          <div class="user_roles_box_top">
                            ...
                            <div class="roles_content">
                              <el-select v-model="industrialPark.currentSelect" class="m-2" :disabled="!isEdit"
                                placeholder="请选择产业园" @change="currentSelectChange(industrialPark, industrialParkList)
                                  ">
                          ...
                          <div class="role_item">
                            ...
                            <div class="roles_content">
                              <el-dropdown :ref="(el) => parkRef(el, index, par)" trigger="contextmenu"
                                placement="bottom-start" :disabled="!isEdit" max-height="176" :teleported="false"
                          ...
                          <!-- <span v-if="industrialParkListArr.length > 1" class="submitIcon" @click="handleRemovePark(index)">
                            <SvgIcon name="del" w="16px" h="16px" />删除
                          </span>
                          <span v-if="index == industrialParkListArr.length - 1" class="submitIcon addIcon"
                            @click="handleAddPark">
                            <SvgIcon name="add" color="white" w="16px" h="16px" />新增
                          </span> -->
                        </div>
                        <div class="role_blank_right">
                          <!-- <div v-if="index === industrialParkListArr.length - 1" class="add_btn" @click="handleAddPark">
                            添加
                          </div>
                          <div class="delete_btn" @click="handleRemovePark(index)">
                            删除
                          </div> -->
                          <span v-if="par.industrialParkListArr.length > 1 && isEdit" class="submitIcon"
                            @click="handleRemovePark(index, par)">
                            <SvgIcon color="var(--default-main-color)" name="del" w="20px" h="20px" />
                          </span>
                          <span v-if="index == par.industrialParkListArr.length - 1 && isEdit" class="submitIcon addIcon"
                      ...
                      <div v-for="(manufacturer, index) in par.manufacturerListArr" :key="manufacturer.id"
                        ...
                        <div class="role_blank_left">
                          <div class="user_roles_box_top">
                            ...
                            <div class="roles_content">
                              <el-select v-model="manufacturer.currentSelect" class="m-2"
                                :disabled="(manufacturerBossFlag && !isEdit) || manufacturer.enableStatus == 2"
                                placeholder="请选择加工商" @change="currentmanufacturerSelectChange(
                                    manufacturer,
                                    manufacturerList
                                  )
                                  ">
                                  ...
                                  <div class="org_role_down_menu_box">
                                    ...
                                    <div class="role_down_menu_list">
                                      <div v-for="item in manufacturerRoleArr" :key="item.id"
                                        ...
                                        <div class="role_down_menu_select_box">
                                          <span class="custom_checked_box" :class="{
                                              checked: industrialManufacturerChecked(item, manufacturer),
                                              // deisable: item.id === '32'
                                            }">
                        ...
                        <div class="role_blank_right">
                          <!-- <span v-if="manufacturerListArr.length > 1 && !manufacturerBossFlag && isEdit" class="submitIcon"
                            @click="handleRemoveManufacturer(index,par)">
                            <SvgIcon name="del" w="16px" h="16px" />删除
                          </span>
                          <span v-if="index == manufacturerListArr.length - 1 && !manufacturerBossFlag && isEdit"
                            class="submitIcon addIcon" @click="handleAddManufacturer(par)">
                            <SvgIcon name="add" color="white" w="16px" h="16px" />新增
                          </span> -->
                          <span v-if="par.manufacturerListArr.length > 1 && isEdit" class="submitIcon"
                            @click="handleRemoveManufacturer(index, par, manufacturer)">
                            <SvgIcon color="var(--default-main-color)" name="del" w="20px" h="20px" />
                          </span>
                          <span v-if="index == par.manufacturerListArr.length - 1 && isEdit" class="submitIcon addIcon"
                  ...
                  <div class="btns" v-if="isEdit && orgList.length > 1">
          ...
          <div class="bottom_box">
            <div class="submitBox">
              <el-button type="primary" class="search_button" @click="handleSave" v-if="isEdit">确认</el-button>
              <el-button class="reset_button" @click="handleEdit()" v-if="!isEdit">编辑</el-button>
              <el-button class="reset_button" @click="handleCancelSave()" v-if="isEdit">取消</el-button>
            </div>
          </div>
        </div>
      </div>
    ...
    <el-dialog v-model="isShowSaveDialog" width="500" :show-close="false" :destroy-on-close="true"
      ...
      <div class="save_dialog_content">
        <div class="save_dialog_body">
          <p class="tips_text">你已修改用户信息，确定保存？</p>
        </div>
...
let isEdit = ref(false) // 第一次进来为查看
let manufacturerBossFlag = ref(false) //判断当前用户是否是加工商老板的角色
...
const getUserDetail = () => {
  ...
  api.getUserDetail(reqData).then((res) => {
    if (res.code !== "000000") {
      return ElMessage.error(res.message);
    }

    const { id, userMobile, userName, userOrgRoleList, userState } = res.data;

    orgValue.value = userOrgRoleList.length
      ? userOrgRoleList[0].orgCode
      : "head_office";

    inituserOrgRoleList(userOrgRoleList);
    Object.assign(userInfoData, {
      id,
      userMobile,
      userName,
      userState,
      userOrgRoleList: userOrgRoleList.map((item) => item.roleId),
    });
  });
};

// 初始化选项数据
const inituserOrgRoleList = (userOrgRoleList) => {
  let headOfficeList = []
  let industrialList = []
  let manufacturerList = []
  userOrgRoleList.forEach(item => {
    console.log(1111)
    if (item.orgCode === 'head_office') {
      console.log(222,item)
      headOfficeList.push(item)
    }
    if (item.orgCode === 'industrial_park') {
      industrialList.push(item)
    }
    if (item.orgCode === 'manufacturer') {
      manufacturerList.push(item)
    }
  })

  if (headOfficeList.length) {
    let headOfficeSelectArr = []
    headOfficeList.forEach(item => {
      headOfficeSelectData.value.push(...item.userRoleList.map(p=>p.roleId));
      headOfficeSelectArr.push(...item.userRoleList.map(i => ({
        orgId: item.orgId,
        id: i.roleId,
        name: i.roleName
      })));
  ...
  if (industrialList.length > 0) {
    let industrialParkListArr = industrialList.map(item => {
      const selectedArr = item.userRoleList.map(i => {
        return {
          id: i.roleId,
          name: i.roleName
        }
    ...
    orgList.value.push({
      orgValue: 'industrial_park',
      industrialParkListArr
    })
  }
  if (manufacturerList.length > 0) {
    let manufacturerListArr = manufacturerList.map((item) => {
      const selectedArr = item.userRoleList.map((item) => {
        if (!manufacturerBossFlag.value && item.pid == '29') { manufacturerBossFlag.value = true, tempFlag.value = true }
        return {
          id: item.roleId,
          name: item.roleName,
        };
    ...
    orgList.value.push({
      orgValue: 'manufacturer',
      manufacturerListArr
    })
  }
  // if (orgValue.value === "head_office") {
  //   console.log(
  //     "inituserOrgRoleList-head_office",
  //     userOrgRoleList,
  //     headOfficeSelectArr
  //   );
  //   if (userOrgRoleList.length > 0) headOfficeSelectArr.value = userOrgRoleList[0].userRoleList.map((item) => {
  //     headOfficeSelectData.value.push(item.roleId);
  //     return {
  //       id: item.roleId,
  //       name: item.roleName,
  //     };
  ...
  // if (orgValue.value === "industrial_park") {
  //   const industrialParkListArrData = userOrgRoleList.map((item) => {
  //     const selectedArr = item.selectedArr.map((item) => {
  //       return {
  //         id: item.roleId,
  //         name: item.roleName,
  //       };
  ...
  //   industrialParkListArr.push(...industrialParkListArrData);
  // }

  // if (orgValue.value === "manufacturer") {
  //   const manufacturerListArrData = userOrgRoleList.map((item) => {
  //     const selectedArr = item.userRoleList.map((item) => {
  //       if (!manufacturerBossFlag.value && item.pid == '29') {manufacturerBossFlag.value = true, tempFlag.value = true}
  //       return {
  //         id: item.roleId,
  ...
  //   manufacturerListArr.push(...manufacturerListArrData);
  // }
  if (!isEdit.value && !tempFlag.value) {
    manufacturerBossFlag.value = true
  }
};
...
const headOfficeData = computed(() => {
  ...
  // if (orgValue.value === "head_office") {
  if (rolesTreeData['head_office'].length) {
    return rolesTreeData['head_office'][0].children;
  } else {
    return [];
  }
  // }
...
const currentmanufacturerSelectChange = (manufacturer, manufacturerList) => {
  ...
  Object.assign(manufacturer, {
    ...
    enableStatus: valData.enableStatus
  });
};
...
const handleManufacturerRemoveTag = (val, manufacturer) => {
  if (!isEdit.value) return
  if (manufacturer.enableStatus == 2) return ElMessage.warning("该加工商处于禁用状态");
  // if (val.id === "32") {
  //   return;
  // }
...
// 移除加工商
const handleRemoveManufacturer = (index, par, manufacturer) => {
...
// 点击展开加工商角色菜单
const handleShowManufacturerRoleMenu = (
  index,
  manufacturer,
  manufacturerList,
  par
...
const getOrgRoleList = () => {
  ...
  //     //如果在flag为true的情况下，有数组为空的数据，不允许提交
  //     if (item.selectedArr.length == 0 && flag) flag = false
  //     const itemData = {
  //       orgCode: "industrial_park",
  //       orgId: item.id,
...
const handleSave = () => {
  ruleFormRef.value.validate((valid) => {
    ...
    // console.log(userOrgRoleList, orgList,'wwww')
    let flag = false
    orgList.value.map(item => {
      if (item.orgValue == '') {
        flag = true
...
const suerSaveRole = () => {
  const { userMobile, userName, userState, id } = userInfoData;

  const userOrgRoleList = getOrgRoleList();
  console.log(orgList, userOrgRoleList)
  if (!userOrgRoleList) {
    return ElMessage.warning("角色信息不能为空");
  }
  const reqData = {
  ...
  api.updateUser(reqData).then((res) => {
    if (res.code !== "000000") {
      // return ElMessage.error(res.message);
      return ElMessage({ message: res.message, type: 'error', duration: 1000 * 5 });
    }
...
const handleEdit = () => {
  isEdit.value = true
  if (!tempFlag.value) {
    manufacturerBossFlag.value = false
  }
}
