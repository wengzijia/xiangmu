...
let listParam = reactive({
    ...
    sortType: 1, // 排序类型：0.按加工商首字母正序，1.按时间(审核列表是按创建时间，其他列表是按启用时间)正序
    pageNumber: 1, // 当前页
    pageSize: 15, // 显示条数
    ruleStatus: "4", // 状态
 ...
 */
const getMergeItem = (data, keyArr) => {
    const spanArr = [];
    var pos = 0;
...
// 表格跳转
const tableJump = (row) => {
    // window.open(`/unitChargeRule/unitChargeRuleCWDetail?id=${row.id}`, '_blank')
