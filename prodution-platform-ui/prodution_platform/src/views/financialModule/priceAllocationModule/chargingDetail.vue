    ...
    <div class="unitChargeDetailContainer">
        ...
        <div class="unitChargeDetailBox">
            <div class="leftContainer">
                <div class="versions">{{ chargingRuleInfo.version }}</div>
                ...
                <div class="detailBox">
                    <div class="titleBox">
                        ...
                        <div>加工商:</div>
                        <div>状态:</div>
                        <div v-if="chargingRuleInfo.ruleStatus == 4">启用时间:</div>
                    </div>
                    <div class="contentBox">
                        <div>{{ chargingRuleInfo.attributeName }}</div>
                        <div>{{ chargingRuleInfo.formulaDesc }}</div>
                        <div>{{ chargingRuleInfo.createTime }}</div>
                        <div>{{ chargingRuleInfo.manufacturerName }}</div>
                        <div :style="{ color: statusColor }">{{ statusDescFn(chargingRuleInfo.ruleStatus) }}</div>
                        <div v-if="chargingRuleInfo.ruleStatus == 4">{{ chargingRuleInfo.enableTime }}</div>
                    </div>
                </div>
                <div class="tableBox">
                    <uniPriceRuleTable :tableData="ruleTableData"></uniPriceRuleTable>
                </div>
                <div class="footer">
                    <div class="submitBox" v-if="route.query.type == 2">
                        <el-button type="primary" class="search_button passAudit" @click="audit(3)">审核通过</el-button>
                        <el-button type="primary" class="noPassAudit" @click="audit(4)">审核不通过</el-button>  
                    </div>
                </div>
            </div>
                    ...
                    <div class="auditFlowBox">
                        ...
                        <div class="flowBox" v-if="recordListData?.length && !showFlag">
                            <el-steps direction="vertical">
                                <el-step v-for="item in recordListData" :key="item.id">
                                    <template v-slot:icon>
                                        <img class="statusImg" :src="getImageUrl(item.auditStatus)" alt="">
                                    </template>
                                    <template v-slot:title>
                                        <div class="itemBox">
                                            <div class="headBox">
                                                <div class="name">{{ item.auditUserName }}-{{ item.roleName }}</div>
                                                <div class="tiemBox">
                                                ...
                                                <div class="resultBox">
                                                    ...
                                                    <div :style="{ color: auditStatuColor }">{{
                        ...
                        <div class="putAwayFlowBox" v-if="recordListData?.length && showFlag">
                            <el-steps direction="vertical">
                                <el-step>
                                    <template v-slot:icon>
                                        <img class="statusImg"
                                            :src="getImageUrl(recordListData[0].auditStatus)" alt="">
                                    </template>
                                    <template v-slot:title>
                                        <div class="itemBox">
                                            <div class="headBox">
                                                <div class="name">{{ recordListData[0].auditUserName }}-{{
                                                    recordListData[0].roleName }}</div>
                                                <div class="tiemBox">
                                                    <div class="title">审核时间:</div>
                                                    <div class="time">2{{ recordListData[0].auditTime }}</div>
                                                </div>
                                                <div class="resultBox">
                                                    <div class="title">审核结果:</div>
                                                    <div :style="{ color: auditStatuColor }">{{
                                ...
                                <el-step>
                                    <template v-slot:icon>
                                        <img class="statusImg"
                                            :src="getImageUrl(recordListData[recordListData.length - 1].auditStatus)"
                                            alt="">
                                    </template>
                                    <template v-slot:title>
                                        <div class="itemBox">
        ...
        <div class="popupContainer">
            <el-dialog v-model="showPop" :show-close="false" :destroy-on-close="true" width="700">
                <template #header>
                    <div class="hintBox">
                        <div class="text" v-if="auditStatus == 3">审核通过</div>
                        <div class="text" v-if="auditStatus == 4">审核不通过</div>
                        <img class="closeImg" src="@/assets/images/close.png" alt="" @click="colsePop">
                    </div>
                </template>
                <div class="hintContent" v-if="auditStatus == 4">是否确认审核不通过？若确认请填写备注</div>
                <el-form :inline="true" ref="ruleFormRef" :model="formParam" :rules="rules" class="infoFormBox"
                    :label-width="auditStatus == 4 ? '' : '82px'">
                    <el-form-item label="启用时间:" prop="time" v-if="auditStatus == 3">
                        <el-config-provider :locale="zhCn">
                            <el-date-picker v-model="formParam.time" :clearable="false" type="date" placeholder="选择时间"
                                popper-class="startTimePopper" />
                        </el-config-provider>
                    </el-form-item>
                    <el-form-item label="备注:" prop="remarks">
                        <el-input v-model="formParam.remarks" placeholder="请在此添加备注" resize="none" show-word-limit
                            type="textarea" class="remarks" maxlength="200"
                            :input-style="{ width: auditStatus == 3 ? '546px' : '596px', height: '120px', color: '#333333' }" />
                    </el-form-item>
                </el-form>
                <div class="btnBox">
...
<script name="unitChargeRuleAuditCWDetail" setup>
import { ElMessage } from "element-plus";
import uniPriceRuleTable from "@/views/unitChargeRule/components/uniPriceRuleTable.vue"
import { getParkDetail, getAuditRecordList, getBillRuleHisPageHttp, auditBillRuleHttp } from "@/api/modules/unitChargeRule"
...
const getAuditRecordListFn = async () => {
    ...
    if (chargingRuleInfo.ruleStatus == 2 && chargingRuleInfo.auditStatus == 1) recordListData.value.push({
        auditUserName: JSON.parse(localStorage.getItem("userInfo")).userName,
        roleName: "产业园财务",
        auditStatus:5
    })
}
...
const getBillRuleHistPageFn = async () => {
    const res = await getBillRuleHisPageHttp({
        attributeId: chargingRuleInfo.attributeId,
        language: "zh-cn",
        manufacturerTypeId: chargingRuleInfo.manufacturerTypeId,
        pageNumber: 1,
        pageSize: 10,
...
// 初始化页面数据
function initPageFn() {
    getDetailInfoFn().then(() => getBillRuleHistPageFn())
...
let auditStatusImg = (val) => {
    if (chargingRuleInfo.ruleStatus == 2 && chargingRuleInfo.auditStatus == 1) {
        auditStatuColor = "#F9B300"
        return "../../../assets/images/yellowRound.png"
    }
    // 1.采购经理通过，2.采购经理不通过，3.产业园财务通过，4.产业园财务不通过
    if (val % 2 === 0) {
        auditStatuColor = "#F14932"
        return "../../../assets/images/redRound.png"
    } else {
        auditStatuColor = "#338F7A"
        return "../../../assets/images/greenRound.png"
    }
}
...
let rules = reactive({
    ...
    remarks: {
        required: computed(() => {
            return auditStatus.value == 4 ? true : false
...
const toPage = () => {
    let pathInfo = router.resolve({ //使用resolve
        ...
        query: {
            ...
            manufacturerTypeId: chargingRuleInfo.manufacturerTypeId, // 加工商类型id
            valuationFormulaId: chargingRuleInfo.valuationFormulaId, // 计价公式id
            version: chargingRuleInfo.version,
            detail: "/financial/chargingDetail"
        }
    })
    ...
    formEl.validate(async (valid, fields) => {
        if (valid) {
            let params = {
                ...
                enableTime: formParam.time && parseTime(formParam.time),
...
const toCurrPage =(row) => {
    // console.log(row, route.query.id)
    if(row.id === route.query.id) return
    openNewPage(`/financial/chargingDetail?id=${row.id}&type=1`)
}
</script>
<style lang="scss">
.discountSelect {
