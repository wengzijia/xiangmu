...
const app = getApp();
import { getShareOrderBillData, setAddOrderPay, getOrderPaySuccess } from "../../../../api/bill";
...
Page({
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          console.log('666')
          wx.navigateTo({
            url: '../../../login/index?shareId=' + _this.data.shareList.share_id+'&shareTypeName=printingModule',
            complete: () => {
              wx.hideLoading()
            }
          })
  ...
  getShareOrderBill() {
    ...
    getShareOrderBillData(_this.data.billDetail).then((resluts) => {
      console.log('分享账单返回数据：----：',resluts)
      if (resluts.code == 200) {
