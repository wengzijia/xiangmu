...
const app = getApp();
import { customerBillMonthDetails, addShareOrderBill } from "../../../api/bill";
...
Page({
  ...
  customerBillMonthDetails() {
    ...
    customerBillMonthDetails(_this.data.billDetail).then((resluts) => {
      if (resluts.code == 200) {
  ...
  addShareOrderBill: function () {
    ...
    addShareOrderBill(data).then((resluts) => {
      ...
      if (resluts.code == 200) {
        ...
        _this.onShareAppMessage = function () {
          ...
          return {
            title: '请接收' + _this.data.supplierName + _this.data.customer.years_month + '月的账单',
            path: '/pages/bill/shareBill/index?shareId=' + _this.data.share_id,
