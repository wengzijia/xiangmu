...
<script>
import { getNowPayQrCode, getOrder, balancePay } from '@/api/order/orderDiff';
import { getUserInfo as getUser } from '@/api/users/userApi.js';
import huabei from './mixins/huabei'; //花呗相关数据
export default {
  mixins: [huabei],
  name: 'component_name',
  data() {
  ...
  created() {
    ...
    this.routerPath = JSON.parse(sessionStorage.getItem('cachePath'));
    // this.isPayType();
    this.getUserInfo();
  },
  beforeDestroy() {
        ...
        if (this.checked === true) {
          ...
          this.$confirm(`您将使用余额支付${useMoney}元`, '支付提醒', {
            ...
            .then(() => {
              this.alicode = '';
              this.wxcode = '';
              this.$router.push({ path: '/service/diffpay', query: { orderNo: this.orderNo, paytype: 'balance' } });
          ...
          this.alicode = '';
          this.wxcode = '';
          this.$router.push({ path: '/service/diffpay', query: { orderNo: this.orderNo, paytype: 'normal' } });
          this.paytype = 'normal';
          this.tabClick();
