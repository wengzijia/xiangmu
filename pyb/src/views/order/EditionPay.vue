          ...
          <div class="payWapper">
            <div class="payWstart" style="margin-top: 50px">
              ...
              <div class="payWsTxt">
                <h3>订单提交成功！请尽快支付！</h3>
                <p class="payWsTxt_p">
                  共1张订单，<router-link to="/user/editionOrderList">订单中心</router-link>。<span class="gree">请在当天内完成支付</span>，待支付：<span class="red">{{ '￥' + activeMoney }}</span>
                </p>
              </div>
            </div>
...
<script>
import { createPayQrCode, getOrderByOrderNo } from '@/api/order/editionOrderApi';
...
export default {
  ...
  methods: {
    ...
    // },
    toOrderCenter() {
      this.$router.push('/user/editionOrderList');
    },
    ...
    getOrderMoney() {
      ...
      getOrderByOrderNo({ data: this.orderNo })
        ...
        .catch(() => {
          this.$router.push({ path: '/user/editionOrderList' });
        });
    },
    // 定时器查询订单状态
    getOrderInfo() {
