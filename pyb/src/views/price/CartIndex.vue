...
import { priceInvoke, editionInvoke } from '@/api/price/priceApi';
import FootNotice from '@/components/footer/FootNotice';
export default {
  components: {
    LoginForm,
    PriceRight,
  ...
  data() {
    return {
      ...JSON.parse(JSON.stringify(defaultFormData)), // 深拷贝
      bindType: 'pinyiban', // 绑定类型,cate:商品计价
      isCommonSize: false
    };
  },
  created() {
    console.log('process配置信息', process);
  ...
  methods: {
    ...
    initPage() {
      ...
      // console.log('获取公用的表单信息', this.commonVoList);
      // 重新设置输入文本框长度
      reSetSize();
      this.clearData();
      ...
      // 获取菜单名称,对应显示在浏览器窗口上
      getSubName();
      // console.log('commonFormData', this.commonFormData);
      // console.log('commonVoList', this.commonVoList);
    ...
    async priceValuation() {
      ...
      const tatolTime = _this.priceFormData.tatolTime;
      ...
      _this.priceFormData.tatolTime = tatolTime;
      //   console.log(' _this.priceFormData.yssl', _this.priceFormData.yssl);
      console.log(' _this.priceFormData.extends', _this.priceFormData.extends);
      //   console.log(' _this.priceFormData.extends', _this.priceFormData.extends);
      // 拼一版计价方法
      let strVal = await commonValuationNew();
      ...
      if (strVal) {
        ...
        if (strEquals(this.cateId, '1')) {
          if (strVal.indexOf('粘合:') === -1 && !strEquals(this.subCateId, '17')) {
            strVal += '粘合:自动粘1处;';
          }
          if (strEquals(this.subCateId, '17')) {
            strVal += '粘合:自动锁底;';
          }
        ...
        editionInvoke({ goodsId: this.formData.goodAttrVo.id, goodsAttr: strVal, number: this.priceFormData.yssl, fnumber: this.priceFormData.tkks, cextends: this.priceFormData.extends, city: city })
          .then(res => {
            console.log('计价信息', res);
            if (res.code === '970006') {
          ...
          .catch(res => {
            ...
            _this.priceFormData.priceProgress = false;
            // _this.$message({ type: 'error', message: '长x宽>5000' });
          });
      } else {
        _this.priceFormData.priceProgress = false;
      }
