            ...
            <template v-for="(gplist, index) in adaptiveMap">
              ...
              <tbody :key="index">
                <tr v-for="(gro, indexs) in gplist" :key="indexs" style="border: 2px solid #7abd54;">
                  ...
                  <td class="numberTd" @click="updateAdaptiveNums(index, indexs)">
                    <el-input-number v-model="gro.num" :min="1" size="mini"></el-input-number>
                  </td>
                  <td class="fnumTd" @click="updateAdaptiveNums(index, indexs)">
                    <el-input-number v-model="gro.fnumber" :min="1" size="mini"></el-input-number>
                  </td>
                  <td>
...
export default {
  ...
  methods: {
    ...
    //修改 已适配数量 款数
    updateAdaptiveNums(index, indexs) {
      var data = {
