    ...
    <template v-for="(groupItem, index) in groupVoList">
      <div :key="'div_' + index" class="hardList">
        ...
        <price-major
          ...
          :bindType="bindType"
        ></price-major>
        <div v-if="strEquals(groupItem.name, '印刷层信息')" class="Pantips" style="margin-left: 130px;"><span class="red">温馨提示:</span> 如果您的设计文件中使用了三色或四色混合(浅灰、灰、深灰、黑)、暗红色、浅红色、墨绿色、蓝紫色、深紫色、深褐色、咖啡色等色，建议您选择专色印刷，否则出现色差问题不予受理。</div>
      </div>
