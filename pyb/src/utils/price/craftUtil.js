...
export function isCopyCraft(code) {
  ...
  } else if (endWith(code, '_BKKZ') || endWith(code, '_BKKZ_COPY')) {
    return true;
  } else if (endWith(code, '_YWBH') || endWith(code, '_YWBH_COPY')) {
    return true;
  } else if (endWith(code, '_TC') || endWith(code, '_TC_COPY')) {
    return true;
  }
  return false;
}
...
export function craftInputAttr(item, groupValueList, inputValues) {
  ...
  if (endWith(item.miniZtAttrGroup.code, 'TJ_TK') || endWith(item.miniZtAttrGroup.code, 'TJ_TK_COPY')) {
    tangjin(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, '_JT') || endWith(item.miniZtAttrGroup.code, '_JT_COPY')) {
    jitu(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, '_JBUV') || endWith(item.miniZtAttrGroup.code, '_JBUV_COPY')) {
    juBuUv(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, '_JA') || endWith(item.miniZtAttrGroup.code, '_JA_COPY')) {
    jiao(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, '_TC')) {
    tiechuang(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, '_TCCC')) {
    tiechuang2(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, 'NTJWKD')) {
    requiredInput(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, 'XGBBFS_ZDY_BB')) {
    requiredInput(item, groupValueList, inputValues, 'MM');
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, '_BSEVA')
    || endWith(item.miniZtAttrGroup.code, '_HSEVA')
    || endWith(item.miniZtAttrGroup.code, '_BSZZM')
    || endWith(item.miniZtAttrGroup.code, '_HSZZM')) {
    ntcl(item, groupValueList, inputValues);
    return true;
  } else if (endWith(item.miniZtAttrGroup.code, 'CS_SZZD')) {
    requiredInput2(item, groupValueList, inputValues);
    return true;
  }
  return false;
}
...
export function tangjin(item, groupValueList, inputValues) {
  let name = item.miniZtAttrGroup.actualValue !== null && item.miniZtAttrGroup.actualValue !== '' ? item.miniZtAttrGroup.actualValue : item.miniZtAttrGroup.name;
  console.log(item, groupValueList);
