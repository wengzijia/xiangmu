...
import { commonViewList, cartCustomCommonList, pitSbhCommonList, tkksCommonItem, tagCommonList, ysslCommonItem, outerBoxAtrrValue, hardcoverBoxCommonList } from './proofCommonListData';
...
export async function commonValuation() {
  ...
  console.log(that.commonFormData.inputValues, '尺寸应输:' + that.sizeInCount, '尺寸实输:' + sizeCount, '总应输:' + totalInCount, '总实输:' + inCount, '是否计算尺寸:' + isComputeSize);
...
export function otherInputChange(val, data) {
  console.log('otherInputChange', val, data);
  // dataKey
  ...
  if (integer.test(data.data[data.dataKey])) {
    if (data.code === 'TKKS') {
      ...
      if (!isReInSize()) {
        for (let i = 0; i < that.commonVoList.length; i++) {
          if (endWith(that.commonVoList[i].code, 'SIZE')) {
            console.log(that.commonVoList[i]);
        ...
        console.log('goodSize', that.formData.goodSize.goodAl * that.formData.goodSize.goodAw);
...
export function getCommonVoList(cateId, subCateId) {
  ...
  if (strEquals(cateId, '2')) {
    newList[1].attrList[0].max = 6;
  }
  if (strEquals(subCateId, '39')) {
    newList[1].attrList[0].max = 5;
  }
  return newList;
}
/**
...
export function getSubName() {
  ...
  setDocumentTitle(that.cateName + '打样-拼一版');
  if (subNav) {
