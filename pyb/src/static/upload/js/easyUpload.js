...
$.fn.easyUpload = function (opts) {
    var defaults = {
        allowFileTypes: '*.pdf;*.mp4;*.png',//允许上传文件类型，格式'*.pdf;*.doc;'
        allowFileSize: 100000,
        selectText: '选择文件',//上传按钮文案
        multi: true,//是否允许多文件上传
        multiNum: 15,//多文件上传时允许的有效文件数
        showNote: true,//是否展示文件上传说明
        note: ' 提示：最多上传15个文件，超出默认前五个，支持格式为：pdf、mp4、png',//文件上传说明
        showPreview: true,
        url: '',//上传文件地址
        fileName: 'Filedata',//文件配置参数
    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += option.multi ? '<span class="easy_upload_head_btn2 noselect">全部删除</span>' : '';
