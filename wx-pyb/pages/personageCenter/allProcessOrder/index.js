...
Page({
    ...
    data: {
        ...data,
        creditType:'edition',
        creditCallBlack:'getOrder',//信用支付回调函数名称
        WXpayOrderSn:'',//合并支付单号
        userInfo: {},
        hasUserInfo: false,
    ...
    onShow: function() {
        ...
        this.setData({
            ...
            height: wx.getSystemInfoSync().windowHeight - ( app.globalData.admin?90:50)
        })
    },
    myNavigateTo() {
        wx.navigateTo({
    ...
    getOrder: function() {
        ...
        switch (this.data.currentTab) {
            ...
            case "2":
                this.setData({
                    regInfo: { pageNumber: 1, pageSize: 20, orderType: this.data.orderType, status: 'confirmed' }, //进行中
                    update: true
                })
                break;
            case "3":
    ...
    totalPay: function(e) {
     ...
     //调用微信支付
     totalWxPay(){
        createWxPayUrl({ orderNo: this.data.WXpayOrderSn, payType: app.globalData.payType }).then(resp => {
