import { countNoPaid,} from "../../../../../api/orderApi";
//index.js
import { deleteOrder, cancelOrder,  createOrder, WxPayUrl, DiffOrderPayUrl,createProxyMergePayQrCode,getAllOrder} from "../../../../../api/orderApi.js";
...
Page({
  ...
  data: {
    ...
    orderData: [],
    height: 550,
    regInfo: { pageNumber: 1, pageSize: 20,status:"shipped" },
    totalPrice: 0,
    list: [],
    Allchecked: false,
  ...
  onLoad(options) {
    ...
    let arr = options.monthly.split('-')
    this.getOrder(Number(arr[0]), Number(arr[1]))
    // 列表
  ...
  getOrder(year,month) {
    // 汇总
    countNoPaid({ orderType: 4,monthly: 1, year: year, month: month,status: "shipped"}).then(res => {
      this.setData({
        summary: res.data
      })
    })
    // 待结
    countNoPaid({ orderType: 4,monthly: 1, balanceStatus: 0, year: year, month: month,status: "shipped"}).then(res => {
      this.setData({
        stay: res.data
      })
    })
    // 待结
    countNoPaid({ orderType: 4,monthly: 1, balanceStatus: 1, year: year, month: month,status: "shipped"}).then(res => {
      this.setData({
        over: res.data
      })
    })
  ...
  loading: function () {
    ...
    if (this.data.orderData.length % 20!==0&&this.data.orderData.length % 20 < 20 ) {
      ...
      console.log(189, this.data.regInfo);
  ...
  getAllOrder: function (exist) {
    ...
    let arr = this.data.monthly.split('-')
    console.log('this.data.monthly', this.data.currentTab);
    //判断请求对应状态的订单
    if (this.data.currentTab != '-1') {
      this.setData({
        regInfo: { pageNumber: 1, pageSize: 20, orderType: 4, balanceStatus: Number(this.data.currentTab), monthly: 1, year: Number(arr[0]), month: Number(arr[1]),status: "shipped"}
      })
    } else {
      this.setData({
        regInfo: { pageNumber: 1, pageSize: 20, orderType: 4,monthly: 1, year: Number(arr[0]), month: Number(arr[1]),status: "shipped" }// 全部
      })
    }
    /**
     * 获取订单列表
...
timestampToTime: function(time) {
  ...
  search: function () {
    ...
    getAllOrder({ orderNo: keywords, ...this.data.regInfo }).then(res => {
      this.setData({
        orderData: res.data.map(item => {
          if (item.miniEditionGoodsInfo) {
            item.miniEditionGoodsInfo.map(e => {
  ...
  totalWxPay(res){
    //调用微信支付
    createProxyMergePayQrCode({ orderNo: this.data.WXpayOrderSn, payType: app.globalData.payType }).then(resp => {
