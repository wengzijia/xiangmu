//index.js
import { ordersDetails, editionFiles, WxPayUrl, confirmReceiver, cancelOrder, del, deleteOrder,DiffOrderPayUrl,repaidByOrderNo,createOrderRefund,delivery } from "../../../../api/orderApi.js";
...
Page({
  ...
  getDelivery(date,time){
    ...
    delivery(data).then(res=>{
      if (res.success) {
        this.setData({
          pickUpDate:res.data.slice(0,10)
        })
      }
    })
  },
      ...
      if (than.data.buttonStatu == 'active') {
        wx.chooseMessageFile({
          ...
          success(res) {
            ...
            if(newfilename.indexOf(".jpg") != -1 || newfilename.indexOf(".png") == -1 ){
              than.alert()
            } else {
              wx.showLoading({
                title: '上传文件中...',
              ...
              wx.uploadFile({  
                url: app.globalData.requestUrl+'/api/consumer/user/miniUserAccountTopup/uploadFile',   //上传的路径                  
