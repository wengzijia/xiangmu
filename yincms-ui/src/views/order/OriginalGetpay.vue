          ...
          <div class="payWapper">
            <div class="payWstart" style="margin-top: 50px">
              ...
              <div class="payWsTxt">
                <h3>订单提交成功！请尽快支付！</h3>
                <!-- <p class="payWsTxt_p">
                  共1张订单，<router-link to="/user/demandlist">订单中心</router-link>。<span class="gree">请在当天内完成支付</span>，待支付：<span class="red">{{ '￥' + money }}</span>
                </p> -->
                <p class="payWsTxt_p">
                  共1张订单，<router-link to="/user/designerlist">订单中心</router-link>。<span class="gree">请在当天内完成支付</span>，待支付：<span class="red">{{ '￥' + money }}</span>
                </p>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    // },
    toOrderCenter() {
      this.$router.push('/user/designerlist');
    },
    ...
    getOrderInfo() {
      ...
      getOrder({ data: this.orderNo })
        .then(res => {
          if (res.code === '000000') {
            ...
            if (res.data.payStatus == 1) {
              ...
              this.timer = setTimeout(_this.getOrderInfo, 1000);
            }
          } else {
        ...
        .catch(() => {
          this.$router.push({ path: '/user/designerlist' });
        });
    },
    // 使用余额支付
    useUserMoney() {
        ...
        if (this.checked === true) {
          ...
          this.$confirm(`您将使用余额支付${useMoney}元`, '支付提醒', {
            ...
            .then(() => {
              this.alicode = '';
              this.wxcode = '';
              this.$router.push({ path: '/service/originalGetpay', query: { ...this.$route.query, paytype: 'balance' } });
          ...
          this.alicode = '';
          this.wxcode = '';
          this.$router.push({ path: '/service/originalGetpay', query: { ...this.$route.query, paytype: 'normal' } });
          this.paytype = 'normal';
          // this.isPayType();
          this.tabClick();
        }
    ...
    paySuccess() {
      ...
      clearTimeout(this.timer);
      if (this.subOrderId && this.orderType != 3) {
        this.$router.push({ path: '/user/designDetail', query: { id: this.subOrderId } });
