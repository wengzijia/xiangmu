        ...
        <div class="uSbox">
          <form class="flex uSbox_form">
            <div class="uSboxL">
              <ul>
                <li>
                  <span class="uSboxL_tx">订单编号：</span>
                  <div class="uSboxL_input">
                    <el-input type="text" name="order_id" class="" placeholder="订单编号" v-model="queryCondition.orderNo" size="small" style="width:160px;" clearable />
                  </div>
                </li>
                <li>
                  <span class="uSboxL_tx">货品名称：</span>
                  <div class="uSboxL_input">
                    <el-input type="text" name="order_id" placeholder="货品名称" v-model="queryCondition.customName" size="small" style="width:150px;" clearable />
                  </div>
                </li>
                <li>
                  <span class="uSboxL_tx">收货人：</span>
                  <div class="uSboxL_input">
                    <el-input type="text" name="consignee_name" placeholder="收货人" v-model="queryCondition.consigneeName" size="small" style="width:110px" clearable />
                  </div>
                </li>
                <li>
                  <span class="uSboxL_tx">订单状态：</span>
                  <div class="">
                    <el-select v-model="queryCondition.status" size="small" style="width:110px">
                      <el-option value="" label="全部"></el-option>
                      <el-option value="1" label="待付款"></el-option>
                      <el-option value="2" label="待确认"></el-option>
              ...
              <ul>
                <li>
                  ...
                  <div>
                    <el-select v-model="queryCondition.payType" size="small" style="width:110px">
                      <el-option value="" label="全部"></el-option>
                ...
                <li>
                  ...
                  <div class="">
                    <el-select name="pay_status" v-model="queryCondition.isPay" size="small" style="width:110px">
                ...
                <li class="dateIput">
                  ...
                  <el-date-picker size="small" type="datetime" v-model="queryCondition.createTime" placeholder="开始时间"></el-date-picker>
                  <span class="uSboxL_tx">-</span>
                  <el-date-picker size="small" type="datetime" v-model="queryCondition.endTime" placeholder="结束时间"></el-date-picker>
                </li>
              </ul>
            </div>
            <div class="uSboxR">
              <el-button icon="el-icon-refresh" size="small" @click="resetSearch">重置 </el-button>
              <el-button icon="el-icon-search" size="small" @click="getAllOrder(queryCondition.status)">搜索 </el-button>
              <!-- s -->
            </div>
          </form>
        </div>
            ...
            <el-checkbox-group v-model="checkList" @change="handleCheckedOrderChange">
              <ul class="close" v-for="order in allOrderData" :key="order.id">
                <ol class="flex itemListT">
                  <p>
                    <span class="recheckbox" v-if="queryCondition.status === '1'">
                      <el-checkbox :label="order.orderNo" :key="order.orderNo" @change="orderchange(order.isCanPay)"><br /></el-checkbox>
                    </span>
                    <span
                      ...
                      <template v-if="order.einvoice != '' && order.einvoice != null">
                        <el-tooltip class="item" effect="light" placement="top-start">
                          <template slot="content">
                            <div class="EattrMtips" v-html="order.einvoice"></div>
                          </template>
                ...
                <li class="ttBox" style="background: #eff2f9">
                    ...
                    <div class="itema" v-for="(orderInfo, index) in order.children" :key="orderInfo.id">
                      <div class="s1">
                        <dl class="flex orderInfoDL">
                          ...
                          <dt class="DLimg">
                            <img v-bind:src="orderInfo.orderDocumentsInfo[0] && orderInfo.orderDocumentsInfo[0].layerImgList[0] ? baseUrl + orderInfo.orderDocumentsInfo[0].layerImgList[0] : orderInfo.coverPath" /><span
                              style="display: none"
                              v-text="index"
                            ></span>
                          </dt>
                          <dd class="orderInfo_t">
                            ...
                            <p class="orderInfo_tC C9" title="">
                              <router-link class="poSn" :to="{ path: '/user/orderdetail', query: { orderNo: order.orderNo, type: '1' } }" target="_blank">{{ orderInfo.poSn }}</router-link>
                            </p>
                            ...
                            <div class="flagDv curr" v-if="orderInfo.sortAdaptive > 0">
                              ...
                              <p></p>
                              <span v-if="!(order.orderType == 1 && order.isPackProof == 0)">适配编码:</span>
                              <p></p>
                              <span v-if="!(order.orderType == 1 && order.isPackProof == 0)" v-text="orderInfo.adaptive"></span>
                              <p></p>
                      ...
                      <div class="s2" v-if="orderInfo.postatus != 'stoped'">
                        <span v-if="orderInfo.goodsId === 959">
                          <div>
                            <span v-if="orderInfo.orderDocumentsInfo.length && orderInfo.orderDocumentsInfo[0].fileUrl != '' && orderInfo.orderDocumentsInfo[0].fileUrl != null">
                        ...
                        <span v-else>
                          <span v-if="orderInfo.orderDocumentsInfo.length && orderInfo.orderDocumentsInfo[0].fileUrl != '' && orderInfo.orderDocumentsInfo[0].fileUrl != null">
                            ...
                            <span v-else>
                              ...
                              <el-button size="small" style="margin: 5px 0" v-if="orderInfo.orderDocumentsInfo[0].fileUrl.lastIndexOf('.cdr') > -1 && !orderInfo.orderDocumentsInfo[0].patoneFile" @click="upcdrimg(orderInfo.id, order.orderNo)"
                                >上传设计稿图片</el-button
                              >
                            </span>
                          </span>
                  ...
                  <div class="s5">
                    <b class="red" v-if="order.bonusUseType === 1">{{ (order.amount * 100 + order.designFee * 100) / 100 }}</b>
                    <b class="red" v-else>{{ (order.amount * 100 - order.bonusMoney * 100 + order.designFee * 100) / 100 }}</b>
                    <p class="C9">(含运费{{ order.shippingFee }})</p>
                    <p class="C9" v-if="order.designFee > 0">(含设计费{{ order.designFee }})</p>
                  </div>
                  <div class="s6">
                  ...
                  <div class="s8">
                    ...
                    <div>
                      <router-link class="s8as" v-if="order.status === 'shipped'" :to="{ path: '/user/orderdetail', query: { orderNo: order.orderNo, type: '1' } }" target="_blank">发货清单</router-link>
                    </div>
                  </div>
                </li>
              </ul>
  ...
  methods: {
    ...
    getAllOrder(status, pageNumber, pageSize) {
      ...
      const reqParam = {
        ...
        customName: this.queryCondition.customName
      };
        ...
        .then(res => {
          ...
          if (res.code === '000000') {
            let list = res.data.map(e=>{
              // 精装盒goodsId
              let isCanOrder = e.children.find(s => s.goodsId=='1620'||s.goodsId=='1619'||s.goodsId=='1617'||s.goodsId=='1616')
              if(isCanOrder) e.canOrder = 0;//订单存在精装盒,不允许翻单
              return e
            })
            this.$set(this, 'allOrderData', list);
    ...
    repurchase(orderNo, citem) {
      let str = '';
      let id = [];
      for (let i = 0; i < citem.length; i++) {
        str += citem[i].goodsAttr;
        id.push(citem[i].goodsId);
      }
      if (str.indexOf('云帆') > -1 && id.indexOf('938') > -1) {
        this.$message({
          type: 'error',
          message: '该材质已下架，请重新下单选择'
        });
        return false;
      }
      var reqparam = {
...
.uSboxR .el-button:first-child {
  margin-bottom: 6px;
}
.uSboxL ul li .uSboxL_input >>> .el-input--small .el-input__inner[type='text'],
.uSboxL ul li >>> .el-select .el-input__inner {
  padding-left: 10px;
}
.uSboxL ul li >>> .el-input__inner{
  border-radius:3px;
}
</style>
