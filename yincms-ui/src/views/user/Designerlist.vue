  ...
  <div>
    ...
    <el-main>
      <div class="right_List">
        <div class="UP_right_List">设计订单</div>
              ...
              <table class="td-void order-tb">
                ...
                <tbody v-for="(order, index) in demandList" :key="index">
                  ...
                  <tr class="tr-th">
                    <td colspan="5">
                      ...
                      <span class="numberTatle"
                        >订单号：<a :href="'/user/designdetail?id=' + order.orderSn" class="orderIdLinks" target="_blank">{{ order.orderSn }}</a></span
                      >
                  ...
                  <tr v-for="(info, index1) in order.proofInfos" :key="index1" class="track57442710364">
                        ...
                        <div class="goodsFont" style="padding-left:15px;max-width:100%">
                          ...
                          <p class="goodsFont_One" style="width:100%;">
                            <a href="javascript:;" class="Hd_goodsName" style="font-weight: 600;font-size: 13px;">
                              <template v-if="order.orderType == 10">平面设计-{{ info.goodsName }}</template>
                              <template v-if="order.orderType == 6">结构设计-{{ info.goodsName }}</template>
                              <template v-if="order.orderType == 7">方案设计-{{ info.goodsName }}</template>
                            </a>
                          </p>
                          <p class="goodsFont_One" style="width:100%;">
                            <a href="javascript:;" class="Hd_goodsName" style="font-weight: 600;font-size: 13px;">
                              <template v-if="order.hardcoverKind == 1">方案设计-提供商品样品</template>
                              <template v-if="order.hardcoverKind == 2">平面设计-提供参照盒子样品</template>
                              <template v-if="order.hardcoverKind == 3">平面设计-提供参照客户文件</template>
                            </a>
                          </p>
                    ...
                    <td v-if="index1 == 0" :rowspan="order.proofInfos.length">
                      <div class="amount amount01">
                        <a :href="'/user/designdetail?id=' + order.orderSn" class="OrderDetail" target="_blank">订单详情</a>
                      </div>
                    </td>
                    <td v-if="index1 == 0" :rowspan="order.proofInfos.length">
                      <div v-if="order.delType == 2 && order.status == 3" class="amount finishOrder finishOrder01">
                        未支付取消
                      </div>
                      <div v-else-if="order.delType == 1 && order.status == 5" class="amount finishOrder finishOrder01">
                        支付取消
                      </div>
                      <div v-else-if="order.delType == 3 && order.status == 3" class="amount finishOrder finishOrder01">
                        系统终止
                      </div>

                      <div v-else-if="order.payStatus == 0">
                        <div v-if="order.amount != 0">
                          ...
                          <div>
                            <div class="amount finishOrder finishOrder01">
                              <a @click="updateProofOrderCancelPaymentFn(order.id)" target="_blank" class="payOrder">取消订单</a>
                            </div>
                          </div>
                        </div>
                      </div>
...
import { getAllProofOrder, adaptationOrder, updateProofOrderCancelPayment } from '@/api/users/userDemandApi';
...
export default {
  ...
  methods: {
    ...
    // 用户 取消支付
    updateProofOrderCancelPaymentFn(id) {
      this.$confirm('确认要取消该订单吗？', '提示', {
        ...
        .then(() => {
          ...
          updateProofOrderCancelPayment({ id: id }).then(res => {
            console.log(res);
            if (res.code == '000000') {
              if (this.payStatus == 0) {
                this.listDemandList({ payStatus: 0, pageNumber: this.pageNumber, pageSize: this.pageSize });
              } else {
                this.listDemandList({ pageNumber: this.pageNumber, pageSize: this.pageSize });
              }
            }
          });
        })
