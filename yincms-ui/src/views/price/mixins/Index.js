...
export default {
  ...
  created() {
    ...
    this.initPage();
    //  名片和联单调用计价接口
    // if (this.$route.query.cateId == '143' || this.$route.query.cateId == '145') {
    //   this.priceValuation();
    // }
  },
  ...
  methods: {
      ...
      for (let i = 0; i < this.commonVoList.length; i++) {
        ...
        for (let j = 0; j < commonVo.attrList.length; j++) {
          ...
          if (attrItem.defaultValue !== null && attrItem.defaultValue !== undefined) {
            // 修改名片和联单印刷数量默认值
            if (this.$route.query.cateId == '145') {
              attrItem.defaultValue = 50;
            } else if (this.$route.query.cateId == '143') {
    ...
    getraradiofn() {
      // if (this.$route.query.isCms) return;
    ...
    async priceValuation(dataval, val, unit, id) {
      ...
      //  return
      //  }
      console.log('sss', dataval, val, unit, id);
      // 传统标签限制间距最大值
    ...
    priceValuationNew: debounce(async function() {
      ...
      if (strVal) {
        ...
        if (strEquals(this.cateId, '10')) {
          ...
          strVal += '品类:固定尺寸手提袋;';
        } else if (strEquals(this.cateId, '100') && strEquals(this.subCateId, '39')) {
          strVal += '品类:吸塑内托;';
          ...
          strVal += '品类:联单;';
        } else if (strEquals(this.cateId, '152')) {
          strVal += '品类:牛油纸;';
        }
        ...
        //   // return
        // }
        if (this.$route.query.subCateId == '29' || this.$route.query.subCateId == '30' || this.$route.query.subCateId == '31') {
          strVal = strVal.replace('印刷类型:拼版印刷;', '');
        }
        // 添加名片尺寸参数
        let cardSize = this.$route.query.cateId == '143' && this.$route.query.subCateId == '144' ? '名片尺寸:90_54;' : '';
        strVal += cardSize;
        ...
        // }
        priceInvoke({ goodsId: this.formData.goodAttrVo.id, goodsAttr: strVal + this.hxid, number: this.priceFormData.yssl, fnumber: this.priceFormData.tkks, cextends: extend, city: city })
          .then(res => {
            if (res.code === '970006') {
              this.formData.showLoginDialog = true;
            } else {
