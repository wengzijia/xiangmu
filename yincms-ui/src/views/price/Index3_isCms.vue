...
export default {
  ...
  created() {
    ...
    if (cateId === '2') {
      // 微坑盒
      this.disabledObj.disabledArr = ['印刷类型', '面纸材质', '纸张克重/厚度', '选择', '见坑纸类型', '里纸', '芯纸'];
    } else if (cateId === '3') {
      // 精品盒
      this.disabledObj.disabledArr = [];
    } else if (cateId === '4') {
      // 内托
      this.disabledObj.disabledArr = ['纸张材质', '纸张材质_选择', '纸张颜色', '纸张克重', '纸张克重/厚度'];
    } else if (cateId === '5') {
      // 标签
      this.disabledObj.disabledArr = ['纸张材质', '纸张材质_选择'];
    } else if (cateId === '6') {
      // 吊牌
      this.disabledObj.disabledArr = ['纸张材质', '纸张厚度/克重', '纸张克重', '单双面印刷', '单双面过胶'];
      this.disabledObj.pDisabledArr = ['纸张克重/厚度', '纸张材质'];
      this.disabledObj.pass = ['表面处理'];
    } else if (cateId === '7') {
      // 手提袋
      this.disabledObj.disabledArr = ['纸张材质', '纸张厚度/克重', '成品尺寸'];
      this.disabledObj.pDisabledArr = ['纸张材质'];
    } else if (cateId === '8') {
      // 胶片盒
      this.disabledObj.disabledArr = ['印刷方式', '材质', '材料厚度'];
    } else if (cateId === '11') {
      // 纸杯
      this.disabledObj.disabledArr = ['规格', '纸杯材质'];
    } else if (cateId === '90') {
      // 纸碗
      this.disabledObj.disabledArr = ['规格', '材质'];
    } else if (cateId === '12') {
      // 纸巾盒
      this.disabledObj.disabledArr = ['纸盒材质', '规格', '抽纸数'];
    } else if (cateId === '100') {
      // 塑料包装-内托
      this.disabledObj.disabledArr = ['材质类型', '材质密度', '密度信息', '材质克重', '材质厚度', '吸塑材质'];
    } else if (cateId === '143') {
      // 名片
      this.disabledObj.disabledArr = ['纸张材质', '表面处理'];
    } else if (cateId === '145') {
      // 名片
      this.disabledObj.disabledArr = ['联单尺寸', '联数'];
    } else if (cateId === '10') {
      // 铁盒
      this.disabledObj.disabledArr = ['尺寸', '材质'];
    }
  }
};
</script>
