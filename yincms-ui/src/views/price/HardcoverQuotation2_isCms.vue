<template>
  <div class="hardcover" v-resize="monResize">
    <el-row>
      <el-col>
        <div class="comBox guigeBox cardBox needhide">
              ...
              <div>
                <div class="laybox hsize" v-for="(v, m) in common" :key="m" v-show="data2.length">
                  <div class="EattrLab" style="line-height: 31px; font-size: 13px">
                    <span class="red" v-if="$route.path == '/isCms/HardcoverQuotation'">*</span>
                    {{ v.name }}
                    <!-- 同款款数问号悬浮提示 -->
                    <el-tooltip v-if="v.name == '同款款数' && $route.path == '/isCms/HardcoverQuotation'" width="200" class="item" effect="light" placement="top-start">
                  ...
                  <el-row class="flex inputBx Layerinpts">
                    <div class="inputBxrow" v-for="(vv, mm) in v.attrList" :key="mm">
                      <el-input v-model="priceFormData[vv.val]" :placeholder="vv.name" size="small" v-if="$route.path == '/isCms/HardcoverQuotation'" @change="sizechange(vv.groupId, '', vv.val, priceFormData[vv.val])">
                        ...
                        <div :class="!citem.miniZtAttrGroup.showName && citem.miniZtAttrGroup.code.indexOf('CHILD') != -1 ? 'laychild childactive3' : 'laychild'">
                          ...
                          <div class="flex" v-else-if="strEquals(citem.miniZtAttrGroup.groupType, 'radio')">
                            ...
                            <span v-else-if="citem.miniZtAttrGroup.code.indexOf('del') > -1">
                              <el-button :disabled="readioDisabled(citem)" type="info" size="mini" round @click="delchild(citem, index, i)">取消面料层</el-button>
                            </span>
                          </div>
                          ...
                          <div class="flex" v-else-if="strEquals(citem.miniZtAttrGroup.groupType, 'checkbox')">
                            <template v-if="citem.miniZtAttrGroup.code.indexOf('parent') > -1">
                              <el-row class="Eattrrow" v-for="(attr, i) in citem.attrList" :key="i">
                                <el-checkbox-button :label="attr.id.toString()" v-model="parentdata" :disabled="dbdfn(attr.name, $route.query.subCateId) || readioDisabled(item)" @change="parentchange($event, attr.id, attr.name)">
                            ...
                            <el-row class="Eattrrow" v-for="cattr in citem.attrList" :key="'attr_' + cattr.id" v-else>
                              ...
                              <el-checkbox-button
                                ...
                                v-model="checkdata[citem.miniZtAttrGroup.id]"
                                @change="childchange($event, cattr.id, index, i, cattr.groupId, citem.miniZtAttrGroup.name)"
                                :disabled="(cattr.status == 0 ? true : false) || noFlatDisabled"
                              >
                                {{ cattr.name }}
                              </el-checkbox-button>
                            </el-row>
                      ...
                      <div class="laybox" v-if="$route.query.subCateId != 129">
                        <div class="laycon">
                          <div class="laychild">
                            ...
                            <div class="flex">
                              <el-radio-group class="EattrMgroup" :disabled="$route.query.flat === 'true'" size="small" v-model="dew" button-style="solid" v-if="$route.query.subCateId == 128 || $route.query.subCateId == 138">
                                <el-row class="Eattrrow">
                                  <el-radio-button size="small" :disabled="$route.query.flat === 'true'" @click.native.prevent="sizechange(100, 1)" label="1"> 露腰 </el-radio-button>
                                </el-row>
                              </el-radio-group>
                              <el-radio-group
                                class="EattrMgroup"
                                :disabled="$route.query.flat === 'true'"
                                size="small"
                                v-model="dew"
                                button-style="solid"
                                @change="sizechange(100)"
                                v-if="$route.query.subCateId == 128 || $route.query.subCateId == 137"
                              >
                                <el-row class="Eattrrow">
                                  <el-radio-button size="small" :disabled="$route.query.flat === 'true'" @click.native.prevent="sizechange(100, 2)" label="2"> 露底 </el-radio-button>
                                </el-row>
                              </el-radio-group>
                            </div>
                          </div>
                        ...
                        <div class="laycon">
                          <div class="laychild">
                            ...
                            <div class="flex">
                              <el-row class="flex inputBx Layerinpts">
                                <div class="inputBxrow">
                                  <el-input v-model="dewinp" placeholder="高" :disabled="$route.query.flat === 'true'" size="small" @change="sizechange(100)">
          ...
          <ProofRight class="fixedstatus" ref="HardRight" :getprice="getprice" :priceFormData.sync="priceFormData" @cartfn="cartfn" :title="this.titleobj" v-else></ProofRight> -->
        </div>
      </el-col>
    </el-row>
...
import hardcover from './mixins/hardcover';
...
export default {
  mixins: [entirety, embossMixins, hardcover],
  components: { ProofDesigner },
  data() {
    return {
      formData: {},
      radiodata: {},
  ...
  created() {
    ...
    if (cateId === '3') {
      // 精装盒
      this.disabledObj.disabledArr = ['尺寸', '面料', '材质', '芯板', '纸板材质', '内衬', '部件信息', '皮壳', '柜桶', '抽屉', '包边高度', '物料编号', 'V槽数量'];
      this.disabledObj.pDisabledArr = ['材质', '纸板材质'];
  ...
  methods: {
    ...
    sizechange(gid, name, val, num) {
      // 大货同款款数限制
      if (val == 'same') {
        if (this.$route.path == '/isCms/HardcoverQuotation') {
          // console.log('num', num);
          if (num < 1) {
            // console.log('输入内容出错');
