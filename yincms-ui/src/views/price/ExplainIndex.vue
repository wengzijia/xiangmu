    ...
    <el-row>
      <el-col v-loading="formData.loading">
        <div class="comBox guigeBox cardBox needhide">
          ...
          <price-right class="fixedstatus" :style="'margin-top:' + marginnum + 'px'" :goodsId="formData.goodAttrVo.id" :priceFormData.sync="priceFormData" :addCart="addCart" :addQuote="addQuote" :priceValuation="priceValuation"></price-right>
        </div>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    async priceValuation(dataval, val, unit, id) {
      // 折页说明书限制折叠次数不能超10折
      if (this.$route.query.subCateId == '47' && val > 10 && id) {
    ...
    priceValuationNew: debounce(async function() {
      ...
      if (isVerify) {
        ...
        //   if (strVal.indexOf('折页方式:对折') !== -1) {
        //     strVal += '折叠次数:1;';
        //   } else if(strVal.indexOf('折页方式:十字折') !== -1||strVal.indexOf('折页方式:二折三页') !== -1) {
        //     strVal += '折叠次数:2;';
        //   }else if(strVal.indexOf('折页方式:三折四页') !== -1){
        //     strVal += '折叠次数:3;';
        //   }else if(strVal.indexOf('折页方式:四折五页') !== -1){
        //     strVal += '折叠次数:4;';
        //   }
        // } else 
        if (strEquals(this.cateId, '9') && strEquals(this.subCateId, '46')) {
        ...
        strVal = strVal + _this.priceFormData.extends + ';';
        let city = '';
