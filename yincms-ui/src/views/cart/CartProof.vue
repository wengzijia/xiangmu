          ...
          <table class="ShopCartTable">
            ...
            <tbody>
              <tr v-for="(vo, index) in cartProofList" :key="index">
                ...
                <td class="numberTd">
                  <el-input-number @change="updateNums(index, vo.cateStr)" @input.native="updateNums(index, vo.cateStr)" :ref="'cartNumInput' + index" v-model="vo.num" :min="1" size="mini" :disabled="vo.cate === 13"></el-input-number>
                </td>
                <!-- <td class="fnumTd">
                  <el-input-number @change="updateNums(index, vo.cateStr)" @input.native="updateNums(index, vo.cateStr)" :ref="'cartfnumTdInput' + index" v-model="vo.fnumber" :min="1" size="mini" :disabled="vo.cate === 13"></el-input-number>
                </td> -->
                <td>
...
import { commonViewList, cartCustomCommonList, ysslCommonItem, tagCommonList, hardcoverBoxCommonList } from '@/utils/proof/proofCommonListData';
...
export default {
  ...
  methods: {
    ...
    // 通过 cate 来获取最大数
    findCateGetCommonList(cate) {
      /**
       * 判断依据对应后端逻辑
       * case 1: 卡盒；case 2：精装盒；case 3：内托；case 4：手提袋；case 5：坑盒；case 6：吸塑；
       * case 7：吊牌；case 8：EVA内托；case 9：海绵内托；case 11：珍珠棉内托；case 12：胶片盒；
       * case 13：外箱
       */
      let commonList = null;
      if (strEquals(cate, '1') || strEquals(cate, '3') || strEquals(cate, '5')) {
        commonList = cartCustomCommonList;
      } else if (strEquals(cate, '2')) {
        commonList = hardcoverBoxCommonList;
      } else if (strEquals(cate, '7')) {
        commonList = tagCommonList;
      } else if (strEquals(cate, '8') || strEquals(cate, '9') || strEquals(cate, '11')) {
        commonList = [ysslCommonItem];
      } else {
        commonList = commonViewList;
      }
      return commonList.find(item => item.code === 'YSSL');
    },
    //修改数量 款数
    updateNums: debounce(function(index, name) {
      let _this = this;
      // 获取最新数据

      let newNum = Number(_this.$refs['cartNumInput' + index][0].displayValue);
      let newFnumber = Number(_this.$refs['cartfnumTdInput' + index][0].displayValue);
      let minNum2 = Number(_this.$refs['cartfnumTdInput' + index][0].min);
      // 获取当前商品的最购买值
      let commonList = this.findCateGetCommonList(_this.cartProofList[index].cate);
      commonList = commonList.attrList.find(item => item.code === 'YSSL');
      let minNum = Number(commonList.min);
      let maxNum = Number(commonList.max);
      if (name === '精装盒' && newNum * newFnumber > 5) {
        this.$message({ type: 'error', message: '打样订单数量不能超过5个' });
        this.cartProofList[index].num = 1;
        this.cartProofList[index].fnumber = 1;
      } else {
        // 判断数量
        this.cartProofList[index].num = _this._changeNumber(newNum, minNum, maxNum);
        // 判断款数
        this.cartProofList[index].fnumber = _this._changeNumber(newFnumber, minNum2);
      }
      console.log('this.cartProofList[index].num', newFnumber, minNum2);
      // 发送请求
      var data = {
    ...
    _changeNumber(val, min = 1, max) {
      if (val < min) {
        val = min;
        this.$message.error(`最小数量不能超过${min}`);
      }
      if (max && val > max) {
        val = max;
        this.$message.error(`最大数量不能超过${max}`);
      }
      return val;
    },
    //去结算
