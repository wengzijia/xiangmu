              ...
              <tr v-for="(vo, index) in cartList" :key="index" style="border: 1px dotted #e9e9e9;">
                ...
                <td>
                  ...
                  <el-input-number
                    ...
                    size="mini"
                  ></el-input-number>
                  <el-input-number @change="updateNums(index)" @input.native="updateNums(index)" :ref="'cartNumInput' + index" v-else-if="vo.num > 1000" v-model="vo.num" :min="100" :step="100" :step-strictly="true" size="mini"></el-input-number>
                  <el-input-number @change="updateNums(index)" @input.native="updateNums(index)" :ref="'cartNumInput' + index" v-else v-model="vo.num" :min="100" :step="50" :step-strictly="true" size="mini"></el-input-number>
                </td>
          ...
          <table class="ShopCartTable" style="margin-top:20px;" v-for="(gplist, index) in adaptiveMap" :key="index">
            ...
            <tbody>
              <tr v-for="(gro, indexs) in gplist" :key="indexs" style="border: 1px dotted #e9e9e9;">
                ...
                <td class="numberTd">
                  <el-input-number @change="updateNums(index, 1, indexs)" @input.native="updateNums(index, 1, indexs)" :ref="'cartAdaptiveNumInput' + index" v-model="gro.num" :min="100" :step="50" :step-strictly="true" size="mini"></el-input-number>
                </td>
                <td class="fnumTd">
...
export default {
  ...
  methods: {
    ...
    //修改数量 款数
    updateNums: debounce(function(index, type, indexs) {
      var data = {};
      let _this = this;

      if (type) {
        let num = _this.$refs['cartAdaptiveNumInput' + index][0].displayValue;
        let minNum = _this.$refs['cartAdaptiveNumInput' + index][0].min;
        this.adaptiveMap[index][indexs].num = Number(num) >= Number(minNum) ? num : minNum;
        // this.adaptiveMap[index][indexs].num = _this.$refs['cartAdaptiveNumInput' + index][0].displayValue
        data = {
