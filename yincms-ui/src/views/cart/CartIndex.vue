  ...
  <div>
    <div class="catbj">
      <div class="container clear">
        ...
        <div class="catBox">
          <div v-if="makeupCount > 1" class="row btnfooter">
            <a href="javascript:;" class="removeMuch"><span class="totalPrice">温馨提示：</span>合版数量必须 >= 2 款盒型，且材质、克重、表面处理必须相同。</a>
            <a href="javascript:;" @click="cartPlan" class="lementBtn">立即拼版报价</a>
          </div>
          <table class="ShopCartTable">
            ...
            <tbody>
              <tr v-for="(vo, index) in cartList" :key="index">
                ...
                <td width="24%" class="danPrice gattr{$vo.cart_id}">
                  {{ vo.goodsAttr }}
                  <p v-if="vo.isUserDefined" style="color:red">该单有自定义工艺，勾选合并生成的订单，需待自定义工艺人工评估报价后，订单重新计价后再一起支付</p>
                </td>
                <td>
                  <!-- 精装盒 @blur="updateNums(index)" @change="updateNums(index)" -->
                  <el-input-number @change="updateNums(index)" @input.native="updateNums(index)" :ref="'cartNumInput' + index" v-if="vo.goodsId == 951" v-model="vo.num" :min="20" :step="1" :step-strictly="true" size="mini"></el-input-number>
                  <!-- 数码标签  装订说明书-->
                  <el-input-number
                    @change="updateNums(index)"
                    @input.native="updateNums(index)"
                    :ref="'cartNumInput' + index"
                    v-else-if="vo.goodsId == 937 || vo.goodsId == 958"
                    v-model="vo.num"
                    :min="1"
                    :step="1"
                    :step-strictly="true"
                  ...
                  <el-input-number
                    ...
                    @change="updateNums(index)"
                    :ref="'cartNumInput' + index"
                    v-else-if="vo.goodsId == 960 || vo.goodsId == 965 || vo.goodsId == 966 || vo.goodsId == 1546 || vo.goodsId == 1547 || vo.goodsId == 1548 || vo.goodsId == 1549 || vo.goodsId == 1550"
                    v-model="vo.num"
                    :min="50"
                    :step="100"
                    :step-strictly="false"
                    size="mini"
                  ></el-input-number>
                  <el-input-number @change="updateNums(index)" @input.native="updateNums(index)" :ref="'cartNumInput' + index" v-else-if="vo.num > 1000" v-model="vo.num" :min="100" :step="100" :step-strictly="false" size="mini"></el-input-number>
                  <el-input-number @change="updateNums(index)" @input.native="updateNums(index)" :ref="'cartNumInput' + index" v-else v-model="vo.num" :min="getLessNum(vo)" :step="100" :step-strictly="false" size="mini"></el-input-number>
                </td>
                <td>
          ...
          <div class="row tablefooter">
            ...
            <a href="/cart/indexAdaptive"><span class="lementBtn">去适配盒型</span></a>
          </div>
        </div>
      </div>
      <!--content-->
...
export default {
  ...
  methods: {
    ...
    updateNums: debounce(function(index) {
      let _this = this;
      console.log('index', _this.$refs['cartNumInput' + index][0]);
      let num = _this.$refs['cartNumInput' + index][0].displayValue;
      let minNum = _this.$refs['cartNumInput' + index][0].min;
      this.cartList[index].num = Number(num) >= Number(minNum) ? num : minNum;
      var data = {
  ...
  async created() {
    ...
    // 判断是否显示左右两边的新年烟花
    // if (this.$route.path == '/index/index' || this.$route.path == '/login' || this.$route.path == '/register') {
    //   this.newHappy = false;
    // }
  }
};
