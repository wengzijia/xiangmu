        ...
        <div class="comBox guigeBox cardBox needhide">
          <div class="guigeLt">
            ...
            <div class="EattrMbox" style="padding-bottom: 0">
              ...
              <div>
                <div class="laybox hsize" v-for="(v, m) in common" :key="m" v-show="data2.length&&v.name!=='同款款数'">
                ...
                <div class="laybox hsize" v-show="data2.length">
                  <div class="EattrLab" style="line-height: 31px; font-size: 13px">
                  </div>
                </div>
              </div>
                ...
                <template v-for="(item, index) in data2">
                  ...
                  >
                    ...
                    <div v-for="(citem, i) in item.attrList" :key="i" class="laybox" v-show="citem.miniZtAttrGroup.actualValue != 'title'">
                        ...
                        <div :class="!citem.miniZtAttrGroup.showName && citem.miniZtAttrGroup.code.indexOf('CHILD') != -1 ? 'laychild childactive3' : 'laychild'">
                          ...
                          <el-row class="flex inputBx Layerinpts" v-if="strEquals(citem.miniZtAttrGroup.groupType, 'input')">
                            ...
                            <el-button
                              ...
                              v-if="
                                (citem.miniZtAttrGroup.name == '数码烫金尺寸' || citem.miniZtAttrGroup.name == '上机烫金尺寸' || citem.miniZtAttrGroup.name == '滚烫尺寸' || citem.miniZtAttrGroup.name == '浮雕烫尺寸') &&
                                  citem.miniZtAttrGroup.code.indexOf('DEL') == -1
                              "
                              @click="addgilding(citem, index, i)"
                              >增加{{ citem.miniZtAttrGroup.parentAttrName == '普通烫金' ? '数码烫金' : citem.miniZtAttrGroup.parentAttrName }}</el-button
                            >
                            <el-button
                              type="info"
                              round
                              ...
                              >删除{{ citem.miniZtAttrGroup.parentAttrName == '普通烫金' ? '数码烫金' : citem.miniZtAttrGroup.parentAttrName }}</el-button
                            >
                            <el-button
                              type="danger"
                              round
                          ...
                          <!-- 单选框 -->
                          <div :class="citem.miniZtAttrGroup.name == '印刷' ? 'flex printing' : 'flex'" v-else-if="strEquals(citem.miniZtAttrGroup.groupType, 'radio')">
                      ...
                      <!-- 烫金颜色提示 -->
                      <div class="laycon childactive3" style="backgroundcolor: #ffffff; font-size: 12px; padding: 0" v-if="citem.miniZtAttrGroup.name == '数码烫金颜色'">
                      ...
                      <div class="laycon childactive" style="text-indent: 5.5em; font-size: 13px" v-if="citem.miniZtAttrGroup.code.indexOf('TZZ_SMUVDY') > -1">
                        <div class="laychild">
                          <div>过油 <span style="border: 1px solid red; padding: 7px 12px; border-radius: 5px; margin-left: 7px; cursor: pointer">数码UV光油</span></div>
                        </div>
                      </div>
                      <div class="laycon childactive" style="text-indent: 5.5em; font-size: 13px" v-if="citem.miniZtAttrGroup.code.indexOf('TZZ_SJUVYS') > -1">
                        <div class="laychild">
                      ...
                      <!-- 反面和背面处理提示 -->
                      <div class="laycon childactive" style="text-indent: 8.8em; font-size: 12px; padding: 0" v-if="citem.miniZtAttrGroup.name == '反面'">
                        <div class="laychild">
                          <div><span class="red">*温馨提示：</span>由于打样与大货生产的工艺不同，同等材质如需背面印刷或其他表处理，打样成品较大货成品厚约0.2mm。</div>
                        </div>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    getdata() {
      ...
      getVoList({
        ...
        for (let index = 0; index < defaultCheckboxArr.length; index++) {
          ...
          if (i != -1) {
            _this.copypart(defaultCheckboxArr[index].id, i, _this.data3[i].miniZtAttrGroup.groupName, _this.data3[i].miniZtAttrGroup.name);
          }
        }
        // },100)
    ...
    initial() {
      ...
      this.$set(this, 'entiretyList', JSON.parse(JSON.stringify(entiretyList.proof)));
      ...
      for (let i = 0; i < this.data.length; i++) {
        ...
        if (groupItem.miniZtAttrGroup.pid == pid && parentAttrId == groupItem.miniZtAttrGroup.parentAttrId) {
          if (groupItem.attrList != null && groupItem.attrList.length > 0) {
            ...
            if (groupItem.miniZtAttrGroup.name == '是否加海绵' && str.indexOf(name) == -1) {
              continue;
            }
      ...
      for (let i = 0; i < this.data2.length; i++) {
        for (let j = 0; j < this.data2[i].attrList.length; j++) {
          for (let var1 in selectSelectValues) {
            ...
            if (attrItem && strEquals(this.data2[i].attrList[j].miniZtAttrGroup.id, attrItem.groupId)) {
              if (attrItem.actualValue) {
                if (attrItem.actualValue.indexOf('滚烫') > -1 || attrItem.actualValue.indexOf('浮雕烫') > -1 || attrItem.actualValue.indexOf('数码烫金') > -1 || attrItem.actualValue.indexOf('上机烫金') > -1) {
      ...
      for (let i = 0; i < this.data2.length; i++) {
        for (let j = 0; j < this.data2[i].attrList.length; j++) {
          ...
          if (item.miniZtAttrGroup.groupType == 'input') {
            ...
            if (item.miniZtAttrGroup.name == '滚烫尺寸' || item.miniZtAttrGroup.name == '浮雕烫尺寸' || item.miniZtAttrGroup.name == '数码烫金尺寸' || item.miniZtAttrGroup.name == '上机烫金尺寸') {
          ...
          for (let var1 in checkBoxSelectValues) {
            ...
            for (let j = 0; j < checkBoxSelectValues[var1].length; j++) {
              ...
              if (attrItem && strEquals(item.miniZtAttrGroup.id, attrItem.groupId)) {
                if (attrItem.actualValue) {
                  console.log('fffffffff', this.data2[i].miniZtAttrGroup.id + '_' + attrItem.actualValue);
                  groupValueList.push(this.data2[i].miniZtAttrGroup.id + '_' + attrItem.actualValue + ';');
                }
              }
            }
    ...
    addgilding(data, pi) {
      ...
      for (let a = 0; a < addlist.length; a++) {
        ...
        if (addlist[a].miniZtAttrGroup.groupType == 'select') {
          str = addlist[a].miniZtAttrGroup.parentAttrName == '普通烫金' ? '数码烫金' : addlist[a].miniZtAttrGroup.parentAttrName;
        }
    ...
    sizechange(gid, name, val, num) {
      ...
      if (val == 'same' || val == 'num') {
        ...
        if (this.priceFormData.long && this.priceFormData.wide && this.priceFormData.hign) {
          if (Number(this.priceFormData['num']) * Number(this.priceFormData['same']) > 5) {
            this.$message({ type: 'error', message: '打样订单数量不能超过5个' });
            this.$set(this.priceFormData, 'same', 1);
            this.$set(this.priceFormData, 'num', 1);
          }
        } else {
          if (num > 1) {
        ...
        this.fnumber = this.priceFormData['same'];
      }
      if (name) {
        if (this.dew == name) {
          this.dew = '';
    ...
    inpchange(event, name, id, index, i) {
      if (name == '尺寸') {
        if (Number(this.maxobj[id]) && Number(this.inputdata[id]) > Number(this.maxobj[id])) {
      ...
      if (name == '包边高度') {
        ...
        if (Number(this.inputdata[gfid + '_' + gid]) && Number(this.inputdata[id]) > Number(this.inputdata[gfid + '_' + gid])) {
          console.log('hhhhhhhhhhhh', Number(this.inputdata[gfid + '_' + gid]), Number(this.inputdata[id]), Number(this.inputdata[gfid + '_' + gid]));
          this.$set(this.inputdata, id, null);
    ...
    fname(id, name, child, index, i) {
      ...
      // console.log('ffffffff',dindex);
      let findname = this.data2[index].attrList[i].attrList[dindex]?.name;
      return { dindex, findex, findname };
    },
    findId(name, fname, child, index, i) {
      let findex = null;
      let farr = null;
      this.data2[index].attrList.forEach((e, fi) => {
        if (e.miniZtAttrGroup.name == name && e.miniZtAttrGroup.actualValue == child) {
          findex = fi;
          farr = e;
        }
      });
      let fobj = this.data2[index].attrList[findex].attrList.filter(v => v.name == fname);
      let gid = fobj[0].groupId;
      let id = fobj[0].id;
      return { findex, gid, id, farr };
    },
    setstatus(index, i, seti, seti2, seti3) {
      // console.log('2222222222',setobj);
      // if(seti=='数码UV油'||seti2=='数码UV油'||seti3=='数码UV油'){
      //   this.$set(setobj.attrList[0],'status',0)
      // }else{
      //   this.$set(setobj.attrList[0],'status',1)
      // }
      // if(seti=='上机UV印刷'||seti2=='上机UV印刷'||seti3=='上机UV印刷'){
      //   this.$set(setobj.attrList[3],'status',0)
      // }else{
      //   this.$set(setobj.attrList[3],'status',1)
      // }
    ...
    btnlimit(index, i, child, attrId, name, groupId) {
      ...
      if (name == '印刷' && child != '特种纸') {
        ...
        // let ilist2 = this.findId('丝印','局部UV', child, index, i);
        // let ilist3 = this.findId('丝印','数码局部UV', child, index, i);
        if (ilist.findname == '数码打印' || ilist.findname == '双面数码打印') {
          ...
          // if(this.checkdata[ilist2.gid].indexOf(ilist2.gid+'_'+ilist2.id)>-1){
          //   this.childchange(false, ilist2.id, index, ilist2.findex, ilist2.gid, '丝印')
          //   let si=this.checkdata[ilist2.gid].indexOf(ilist2.gid+"_"+ilist2.id)
          //   this.checkdata[ilist2.gid].splice(si,1)
          // }
        ...
        } else if (ilist.findname == '上机UV印刷') {
          // if (this.radiodata[this.data2[index].attrList[ilist.findex].miniZtAttrGroup.id]) {
          //   this.radiocancel(this.data2[index].attrList[ilist.findex].miniZtAttrGroup.id, index, ilist.findex, this.data2[index].attrList[ilist.findex].miniZtAttrGroup.actualValue, '过油覆膜');
          // }
          // this.setstatus(index, ilist.findex, 'UV油','上机UV印刷');
      ...
      if (name == '过油覆膜') {
        ...
        if (gobj.findname == '普通油' || gobj.findname == 'UV油' || gobj.findname == '数码UV油') {
          if (gobj.findname == 'UV油' || gobj.findname == '数码UV油') {
            this.setall('压纹', child, index, 0);
          } else {
            this.setall('压纹', child, index, 1);
          }
          this.setCheckStatus(index, '丝印', child, 0, gobj.findex, '局部UV', '七彩UV', '数码局部UV');
        } else {
          this.setall('压纹', child, index, 1);
          this.setCheckStatus(index, '丝印', child, 1, gobj.findex, '局部UV', '七彩UV', '数码局部UV');
        }
      }
    },
    ...
    loadDefaultGroup(pid, parentAttrId, index) {
      for (let i = 0; i < this.data.length; i++) {
        ...
        if (this.data[i].miniZtAttrGroup.pname == '过油覆膜' && this.data[i].miniZtAttrGroup.parentAttrId == parentAttrId) {
          this.$set(this.radiodata, this.data[i].miniZtAttrGroup.id, this.data[i].attrList[0].groupId + '_' + this.data[i].attrList[0].id);
        }
      ...
      if (this.datastatus() && this.datastatus2() && this.priceFormData.long && this.priceFormData.wide && this.priceFormData.hign && this.priceFormData.num && this.priceFormData.total_price > 0) {
        ...
        const param = {
          miniCart: {
            ...
            ischeck: 1,
            isfrom: 2,
            totalTime: this.priceFormData.time * 24,
    ...
    getprice: debounce(async function() {
      ...
      if (this.datastatus() && this.datastatus2() && this.priceFormData.long && this.priceFormData.wide && this.priceFormData.hign && this.priceFormData.num && this.fnumber > 0) {
        ...
        let zkcc = '成品外尺寸:' + setdecimal(this.priceFormData.long) + '_' + setdecimal(this.priceFormData.wide) + '_' + setdecimal(this.priceFormData.hign) + ';打样:需要;' + lstr + kstr + (this.dew && this.dewinp ? dewinp : '');
...
.printing .EattrMgroup .Eattrrow:nth-child(n + 4) {
  margin-top: 10px;
}
.cardtabT {
