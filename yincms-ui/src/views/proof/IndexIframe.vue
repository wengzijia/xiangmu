    ...
    <el-row>
      <el-col v-loading="formData.loading">
        <div class="comBox guigeBox cardBox needhide">
          <div class="guigeLt">
            <h1 class="flex guigeH1" v-if="false">
            ...
            <!-- 公用部分 -->
            <proof-common :number-change="numberChange" :otherInputChange="otherInputChange" :form-data.sync="formData" :common-form-data.sync="commonFormData" :common-vo-list.sync="commonVoList" :price-valuation="priceValuation"></proof-common>
            <!-- 专业报价 -->
            <proof-major
          ...
          <proof-right v-show="false" :priceFormData.sync="priceFormData" :addCart="addCart" :priceValuation="priceValuation"></proof-right>
        </div>
...
import ProofRight from '@/components/proof/ProofRight';
import ProofMajor from '@/components/proof/ProofMajor';
import ProofCommon from '@/components/proof/ProofCommon';
...
import entiretyIframe from '@/views/price/mixins/entiretyIframe';
export default {
  ...
  created() {
    ...
    //   this.$router.push({ path: '/proof/index?cateId=1&subCateId=15' });
    // }
  ...
  methods: {
    ...
    initPage() {
      ...
      this.$store.dispatch('GetProofCateNavs').then(res => {
        getSubName();
      });
    },
    /**
     ...
     * 计价方法
     */
    async priceValuation(val, unit, id) {
      // 坑纸内托粘合限制
      if (this.$route.query.subCateId == '34' && val > 3 && unit == '粘位' && id) {
    ...
    async priceValuationNew() {
      ...
      if (strVal) {
        ...
        if (
          strEquals(this.cateId, '1') ||
          strEquals(this.cateId, '2') ||
          strEquals(this.cateId, '8') ||
          strEquals(this.cateId, '6') ||
          strEquals(this.cateId, '7') ||
          (strEquals(this.cateId, '4') && strEquals(this.subCateIds, '34')) ||
          strEquals(this.cateId, '3')
        ) {
          strVal += '是否割样:是;';
        ...
        if (strEquals(this.cateId, '70')) {
          strVal = getAttrValue(this.cateId, this.subCateId, strVal);
        }
        strVal = this.sortAttrStr(strVal, '品类:');
        console.log(strVal);
        var numFilters = await numFilter(this.commonVoList[1].attrList[0], dataResult, this, this.priceFormData.yssl);
        if (!numFilters) {
          _this.priceFormData.priceProgress = false;
          return false;
        }
        proofGetPrice({ goodsId: this.formData.goodAttrVo.id, goodsAttr: strVal, number: this.priceFormData.yssl, fnumber: this.priceFormData.tkks, type: this.paperId })
          .then(res => {
            if (res.code === '970006') {
