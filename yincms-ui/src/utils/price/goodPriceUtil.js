...
export function addCopyGroupCk(parentAttrId, flag) {
  ...
  for (let i = 0; i < that.formVoList.length; i++) {
    ...
    if (parentAttrId === groupItem.miniZtAttrGroup.parentAttrId) {
      ...
      // 寻找子级数据
      that.voList.forEach(item => {
        if (groupItem.miniZtAttrGroup.id == item.miniZtAttrGroup.pid) {
          // 阻止重复添加
          if (endWith(item.miniZtAttrGroup.code, '_COPY')) {
            return;
          }
          // 添加编码是为了阻止重复添加
          item.miniZtAttrGroup.code = item.miniZtAttrGroup.code + '_COPY';
          childData.push(item);
        }
      });
  ...
  if (childData) {
    childData = JSON.parse(JSON.stringify(childData));
    childData.forEach(item => {
      item.miniZtAttrGroup.sortOrder = maxSort + 1;
      item.miniZtAttrGroup.id = item.miniZtAttrGroup.id + that.copyIdStart;
      item.miniZtAttrGroup.parentAttrId = item.miniZtAttrGroup.parentAttrId + that.copyIdStart;
      item.miniZtAttrGroup.pid = item.miniZtAttrGroup.pid + that.copyIdStart;
      item.miniZtAttrGroup.actualValue = item.miniZtAttrGroup.actualValue?.replace('1', '2');
      item.attrList.forEach(item => {
        item.id = item.id + that.copyIdStart;
        item.groupId = item.groupId + that.copyIdStart;
      });
    });
    that.voList.push(...childData);
  }
  // 遍历复制的属性对复制属性进行修改
    ...
    if (that.formVoList[i].miniZtAttrGroup.parentAttrId === parentAttrId) {
      if (copyIndex) {
        ...
        if (isCopy && endWith(that.formVoList[i].miniZtAttrGroup.code, '_COPY')) {
          // 删除子级数据
          let filterData = that.formVoList.filter((item, index) => {
            if (item.miniZtAttrGroup.pid == that.formVoList[i].miniZtAttrGroup.id) {
              that.formVoList.splice(index, 1);
            }
          });
          that.formVoList.splice(i, 1);
          i--;
          ...
          // 删除联动烫金
          if (that.formVoList[i].miniZtAttrGroup.name == '烫金') {
            removeSubGroup(that.formVoList[i].miniZtAttrGroup.parentAttrId);
          } else {
            that.formVoList.splice(i, 1);
          }
          i--;
...
export async function commonDataChange() {
    ...
    for (let var1 in radioSelectValues) {
      ...
      if (strEquals(that.formVoList[i].miniZtAttrGroup.id, attrItem.groupId)) {
        if (attrItem.actualValue) {
          ...
          if (attrItem.remark && (that.formVoList[i].miniZtAttrGroup.name === '吊扣尺寸' || that.formVoList[i].miniZtAttrGroup.name === '吊扣系列')) {
  ...
  if (that.formData.yslx === 'dm') {
    ...
    if ((that.formData.dmys.jybm || that.formData.dmys.jybm2) && that.formData.dmys.jybmVal) {
      // if(that.$route.query.cateId==1&&strValue.indexOf("印刷颜色:无;")>0){
      //   console.log('无')
      // }else{
      //   strValue += '加印白墨;';
      // }
  ...
  if ((that.formData.checkBoxSelectValues?.['4858']?.indexOf('4858_23134') > -1||that.formData.checkBoxSelectValues?.['5953']?.indexOf('5953_53504') > -1) && that.formData.bqys.scCount > 0) {
    strValue += await getPantoneStr(that.formData.bqys, '裱卡专色1');
  }
  // if((that.cateId==1||that.cateId==2)&&(strValue.indexOf('专色:')>-1||strValue.indexOf('裱卡专色')>-1)){
  //   strValue += '跟实物样:是;跟样类型:调色收费;服务:调色跟样;';
  // }
  return strValue;
}
  ...
  if (strEquals(that.cateId, '13')) {
    ...
    for (let i = 0; i < that.formVoList.length; i++) {
      for (let var1 in that.formData.radioSelectValues) {
        ...
        // const groupItem = getGroupById(groupId);
        const attrItem = getGroupAttr(attrId, groupId);
        // console.log('attrItem.code',attrItem.code);
...
export async function commonValuation() {
  ...
  if (that.formData.ysm.list && that.$route.query.cateId == 70 && that.$route.query.subCateId != 85) {
...
export function reSetSize() {
  ...
  // if (that.formData.ysm.scCount && that.$route.query.cateId == 70 && that.$route.query.subCateId != 85) {
...
export function checkboxSelect(param) {
  ...
  if (isAdd) {
    ...
    if (endWith(attrItem.code, '_TJ')) {
      ...
      if (that.$refs['PriceMajor']) that.$set(that.$refs['PriceMajor'], 'isAddBK', true);
    ...
    // 判断是否是增加工艺的选择修改
    if (that.$refs['PriceMajor'] && that.$refs['PriceMajor'].isAddBK && endWith(groupItem.miniZtAttrGroup.code, 'ZJGY')) {
      // 当前没有选中，吧双面印刷和背面处理的禁止选中状态取消
      if (event.length == 0) {
        that.$set(that.$refs['PriceMajor'], 'isAddBK', false);
      } else {
        ...
        that.$set(that.$refs['PriceMajor'], 'isAddBK', id ? true : false);
      }
    }
...
export function radioSelect(event, flag, nid) {
  ...
  if (event) {
    ...
    if (event == '6448_29118') {
      // 专色数量值重置为0
      that.formData.dmys.scCount = 0;
    }
    // 无
    if (event == '6448_65444') {
      // 专色数量值重置为1
      that.formData.dmys.scCount = 1;
    }
    // PANTONE色号置为空
    that.formData.dmys.pantoneList = [];
  }
  // 不是牛皮纸选项初始化
  if (that.$route.query.cateId == 1 && that.formVoListId.indexOf('4757') != -1 && that.formVoListId !== '4757_22880') {
    that.priceFormData.extends = '1';
    that.isShort = false;
  } else {
  ...
  if (that.$route.query.subCateId == '42' && that.formData.checkBoxSelectValues['15299'] && that.formData.checkBoxSelectValues['15299'].indexOf('15299_48591') > -1) {
  ...
  if (that.$route.query.subCateId == 28 && event == '5896_32267') {
  ...
  let zjybm = false;
  let fjybm = false;
  if (groupId == '7014' || groupId == '6775' || groupId == '19799' || groupId == '5994') {
    jybm2 = that.formData.dmys.jybmVal;
    zjybm = that.formData.zmys.jybmVal;
    fjybm = that.formData.fmys.jybmVal;
  }
  let isRemove = 0;
  for (let p in that.formData.radioSelectValues) {
    ...
    if (
      ...
      // 微坑盒金银卡纸显示加印白墨
      strEquals(attrItem.code, 'MZCZ_JYKZ') ||
  ...
  that.formData.zmys.jybm2 = zjybm;
  that.formData.fmys.jybm2 = fjybm;
  that.formData.dmys.jybmVal = jybm2;
  that.formData.zmys.jybmVal = zjybm;
  that.formData.fmys.jybmVal = fjybm;
  toGroupVoList();
  if (that.$route.query.cateId == 1) {
    // if(that.$route.query.subCateId==24&&event=='7014_30896'){
    //   that.priceFormData.extends='0'
    //   that.zhupdatefalse()
    // }else if(that.$route.query.subCateId==24&&event=='7014_30895'){
    //   that.zhupdatetrue()
    // }
    that.getpaperId();
    that.priceValuation2();
...
export function inputSelect(f, val, attrs, attrid) {
  that.priceValuation(f, val, attrs, attrid);
}
  ...
  if (integer.test(data.data[data.dataKey])) {
    if (data.code === 'TKKS') {
        ...
        if (that.subCateId == '85') {
          ...
          if (that.formData.goodSize.goodAl > 0) {
            if (that.formData.goodSize.goodAl * that.formData.goodSize.goodAw * parseInt(val) > maxNum) {
              data.data[data.dataKey] = '1';
              if (that.$route.query.isCms) {
                that.priceFormData.total_price = 0;
                if (that.cateId == 1) {
      ...
      if (strEquals(that.subCateId, '51') || strEquals(that.subCateId, '120')) {
        ...
        // }
      } else if (strEquals(that.subCateId, '36') || strEquals(that.subCateId, '37') || strEquals(that.subCateId, '38')) {
        if (parseInt(val) < 100) {
          that.$message({ type: 'error', message: '100个起印' });
          data.data[data.dataKey] = '100';
        }
...
export function numberChange(val, data) {
  ...
  if (that.$route.query.subCateId == '24') {
    if (data.code == 'goodAl') {
  ...
  if (strEquals(data.code, 'goodAx') && parseFloat(val) === 0) {
    ...
    if (data.max && parseInt(data.data[data.dataKey]) > data.max && !endWith(that.cateId, '2') && !endWith(that.cateId, '1')) {
      // that.$message({ type: 'error', message: groupItem.miniZtAttrGroup.name + '不能大于' + attrItem.maxValue });
...
export function getCommonSuffix(data) {
  that.formData.appendsDesc = {
    goodAl: 'x',
    // 精装盒牛油纸宽单位
    goodAw: that.$route.query.cateId == '152' ? 'mm' : 'x',
    goodAd: 'x',
    goodAt: 'x',
    goodAx: 'x',
    goodAh: 'x',
...
export function addCart(type) {
  let status = false;
  let parentsId = store.getters.parentsId;
  let priceCateNavs = store.getters.priceCateNavs[parentsId].children;
  console.log(priceCateNavs);
  for (let i = 0; i < priceCateNavs.length; i++) {
    // console.log(priceCateNavs[i].data.id);
    if (priceCateNavs[i].data.id == that.cateId) {
      for (let n = 0; n < priceCateNavs[i].children.length; n++) {
        if (priceCateNavs[i].children[n].data.id == that.subCateId) {
          // console.log(priceCateNavs[i].children[n].data.id);
          status = true;
        }
      }
    }
  ...
  if (!status) {
    that.$message({ type: 'error', message: '该盒型不存在' });
    return;
  }
  ...
  setTimeout(function() {
    if (!that.priceFormData.priceProgress) {
      ...
      if (!that.formData.goodAttr) {
        ...
        // 用户行为记录
        let actionId = 3;
        if (that.$route.path == '/price/cartonIndex' || that.$route.path == '/price/cartIndex') {
          actionId = that.tabType == '0' ? 6 : 7;
        }
        if (type == '1') {
          actionId = 2;
          if (that.$route.path == '/price/cartonIndex' || that.$route.path == '/price/cartIndex') {
            actionId = that.tabType == '0' ? 4 : 5;
          }
        }
        that.$report({
          channel: 111000 + Number(that.subCateId),
          actionId,
          param: JSON.stringify({
            goodsId: that.formData.goodAttrVo.id,
            price: that.priceFormData.total_price,
            num: that.priceFormData.yssl
          })
        });
      }
...
export function getCommonVoList(cateId, subCateId) {
  ...
  } else if (strEquals(cateId, '152') && strEquals(subCateId, '153')) {
    newList = JSON.parse(JSON.stringify(cattleList));
...
export async function getCateVoList(bindId) {
  ...
  await getVoList({
    ...
    whereGoodsId: that.$route.query.gid
  }).then(res => {
    that.voList = [];
    that.formVoList = [];
    ...
    if (that.$route.query.subCateId == '29' || that.$route.query.subCateId == '30' || that.$route.query.subCateId == '31') {
      radioSelect('18450_58522');
    }
  });
  if (that.$route.query.subCateId == 88 && that.$route.query.egg) {
    that.SelectEgg();
  }
}
/**
...
export function isReInSize() {
  if (
    ...
    (strEquals(that.cateId, '152') && strEquals(that.subCateId, '153')) ||
