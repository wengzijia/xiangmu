 ...
 */
export function calcNetopitSize(hardcover_size, entityAttr, length, width, height, voList, selectValues, gid = '') {
  // elementList = voList;
  // debugger;
  ...
  let gaObj = [];
  let galObj = [];
  if (gid && (gid == '1551' || gid == '1552' || gid == '1553' || gid == '1554')) {
    gaObj = getMapValue([valuesList[valuesList.length - 2], valuesList[valuesList.length - 1]], 'NTZZCZ');
    galObj = getMapValue([valuesList[1], valuesList[2]], 'WHZZCZ');
  } else {
    gaObj = getMapValue([valuesList[0], valuesList[1]], 'NTZZCZ');
    galObj = getMapValue([valuesList[valuesList.length - 2], valuesList[valuesList.length - 1]], 'WHZZCZ');
  }
