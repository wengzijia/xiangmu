  ...
  await getVoList({
    ...
    .then(res => {
      if (res.success) {
        ...
        console.log(' that.formVoList', that.formVoList);
      } else {
...
export function dataChange() {
  ...
  for (let i = 0; i < that.formVoList.length; i++) {
    for (let var1 in selectSelectValues) {
      ...
      if (strEquals(that.formVoList[i].miniZtAttrGroup.id, attrItem.groupId)) {
        if (attrItem.actualValue) {
          if (that.$route.query.cateId == '3') {
            if (
              that.formVoList[i].miniZtAttrGroup.name == '皮壳纸板克重' ||
              that.formVoList[i].miniZtAttrGroup.name == '柜桶纸板克重' ||
              that.formVoList[i].miniZtAttrGroup.name == '天盖纸板克重' ||
              that.formVoList[i].miniZtAttrGroup.name == '天地盒纸板克重'
            ) {
              that.tparam.t1 = attrItem.name;
            } else if (that.formVoList[i].miniZtAttrGroup.name == '皮壳纸板材质' || that.formVoList[i].miniZtAttrGroup.name == '柜桶纸板材质' || that.formVoList[i].miniZtAttrGroup.name == '天盖纸板材质') {
              that.tparam.t2 = attrItem.name;
            } else if (that.formVoList[i].miniZtAttrGroup.name == '内盒纸板克重' || that.formVoList[i].miniZtAttrGroup.name == '地盒纸板克重' || that.formVoList[i].miniZtAttrGroup.name == '天地盒纸板克重') {
              that.tparam.t3 = attrItem.name;
            } else if (that.formVoList[i].miniZtAttrGroup.name == '内盒纸板材质' || that.formVoList[i].miniZtAttrGroup.name == '地盒纸板材质') {
              that.tparam.t4 = attrItem.name;
            }
          }
...
export function inputChange(param) {
  ...
  if (isNaN(parseFloat(param.value))) {
    ...
    if (attrItem.maxValue || attrItem.minValue) {
      if ((parseFloat(attrItem.maxValue) == 0 || parseFloat(param.value) <= parseFloat(attrItem.maxValue)) && parseFloat(param.value) >= parseFloat(attrItem.minValue)) {
        that.changeParam();
      } else {
        if (attrItem.maxValue && parseFloat(param.value) > parseFloat(attrItem.maxValue)) {
          that.$message({ type: 'error', message: groupItem.miniZtAttrGroup.name + '不能大于' + attrItem.maxValue });
          that.$set(that.formData.inputValues, param.id, attrItem.maxValue + '');
        } else if (parseFloat(attrItem.minValue) && parseFloat(param.value) < parseFloat(attrItem.minValue)) {
          that.$message({ type: 'error', message: groupItem.miniZtAttrGroup.name + '不能小于' + attrItem.minValue });
          that.$set(that.formData.inputValues, param.id, attrItem.minValue + '');
        }
        that.changeParam();
...
export function loadFormGroup(pid) {
  for (let i = 0; i < that.voList.length; i++) {
    ...
    if (strEquals(groupItem.miniZtAttrGroup.pid, pid)) {
      ...
      } else if (strEquals(groupItem.miniZtAttrGroup.groupType, 'select') && groupItem.attrList.length > 0) {
        let attr = getDefaultAttr(groupItem.attrList, groupItem.miniZtAttrGroup.defaultSelect, true);
        if (attr === null) {
          attr = groupItem.attrList[0];
        }
        console.log('groupItem.attrList', groupItem.attrList);
        loadDefaultGroup(attr.groupId, attr.id);
