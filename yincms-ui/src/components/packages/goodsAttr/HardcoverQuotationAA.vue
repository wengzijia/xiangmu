              ...
              <el-form class="EattrMain attr-from" ref="formData" :model="formData" @submit.native.prevent size="small">
                <template v-for="(item, index) in data2">
                  <el-form-item
                    ...
                    v-if="item.miniZtAttrGroup.isPack == 1 || true"
                  >
                    ...
                    <div v-for="(citem, i) in item.attrList" :key="i" class="laybox">
                      <div v-if="citem.miniZtAttrGroup.isPack == 1 || true" v-show="citem.miniZtAttrGroup.actualValue != 'title'">
                        ...
                        <div :class="citem.miniZtAttrGroup.showName ? 'laycon' : 'laycon childactive'" v-if="citem.miniZtAttrGroup.reminder != 'hide'" :style="i == 0 ? 'border-top:1px solid #eee' : ''">
                          <div :class="!citem.miniZtAttrGroup.showName && citem.miniZtAttrGroup.code.indexOf('CHILD') != -1 ? 'laychild childactive3' : 'laychild'">
                            ...
                            >
                              ...
                              <span class="titlename">{{ citem.miniZtAttrGroup.name }}{{ citem.miniZtAttrGroup.actualValue }}</span>
                            ...
                            <div class="flex" v-else-if="strEquals(citem.miniZtAttrGroup.groupType, 'radio')">
                              ...
                              >
                                <el-row class="Eattrrow" v-for="attr in citem.attrList" :key="'attr_' + attr.id">
                                  <template v-if="attr.isPack == 1 || true">
                            ...
                            <div class="flex" v-else-if="strEquals(citem.miniZtAttrGroup.groupType, 'checkbox')">
                              <template v-if="citem.miniZtAttrGroup.code.indexOf('parent') > -1">
                                <el-row class="Eattrrow" v-for="(attr, i) in citem.attrList" :key="i">
                                  <el-checkbox-button :label="attr.id.toString()" v-if="attr.isPack == 1 || true" v-model="parentdata" :disabled="dbdfn(attr.name, subCateId)" @change="parentchange($event, attr.id, attr.name)">
                            ...
                            <div v-else-if="strEquals(citem.miniZtAttrGroup.groupType, 'select')">
                              ...
                              >
                                <template v-for="(attr, aindex) in citem.attrList">
                                  <el-option v-if="attr.isPack == 1 || true" :key="aindex" :label="attr.name" :value="attr.groupId + '_' + attr.id"> </el-option>
                                </template>
                              </el-select>
                              <!-- {{citem.miniZtAttrGroup.id}} -->
                            </div>
  ...
  methods: {
    ...
    getdata() {
      ...
      getVoList({
        ...
        // this.titleobj = getSubNav(this.$store.getters.priceCateNavs, this.cateId, this.subCateId, 1);
        // } else {
        //   this.titleobj = getSubNav(this.$store.getters.proofCateNavs, this.cateId, this.subCateId);
        // }
        ...
        let radiochangeArr = [];
        let goodsAttr = `成品外尺寸:100_100_100;打样:需要;组装工艺:粘接;|#|部件:皮壳1;面料1定型模切:面板;面料1材质:双铜纸_157;面料1印刷:普通印刷;面料1印刷颜色:单色_品红;面料1过油:哑油;芯板纸板定型模切:纸板;芯板纸板材质:粉灰纸板_800;内衬定型模切:面板;内衬材质:拉丝银_215;内衬印刷:UV印刷;内衬印刷颜色:多色_CMYK;内衬过油:UV光油;配件蝴蝶结样式来源:傲彩标准;配件蝴蝶结规格:100_50;皮壳1尺寸:400_100;面料1丝印七彩UV尺寸1:七彩UV_44_55;面料1丝印发泡尺寸1:发泡_655_66;面料1丝印冰花油尺寸1:冰花油_22_11;面料1丝印皱纹油尺寸1:皱纹油_22_11;芯板纸板定型激光雕刻:激光雕刻_34;芯板纸板定型V槽:V槽_4;海绵卡尺寸:1_2;海绵卡材质:粉灰纸_250;海绵卡定型模切:面板;海绵尺寸:1_2;海绵厚度:3;内衬丝印七彩UV尺寸1:七彩UV_1_1;内衬定型激光雕刻:激光雕刻_11;裱合工艺车线:2_2;裱合工艺电脑绣花:3;配件蝴蝶结只数:1;配件蝴蝶结物料编号:1;面料1专色:4,111,222,333,44;面料1烫金1:金色_11_1222;内衬滚烫1:金色_1_3;裱合工艺:裱纸;|#|部件:面顶贴1;面料1定型模切:面板;面料1材质:双铜纸_128;面顶贴1尺寸:100_100;|#|部件:内盒1;面料1定型模切:面板;面料1材质:双铜纸_128;内盒1尺寸:90_94_94;面料1包边:包到底;糊盒:机器;|#|部件:内围框1;面料1定型模切:面板;面料1材质:双铜纸_128;内围框1尺寸:348.9_98.36;`;
        function test() {
          // let goodsAttr =  "成品外尺寸:330_220_110;打样:需要;组装工艺:粘接;|#|部件:皮壳1;面料1定型模切:面板;面料1材质:双铜纸_157;面料1印刷:普通印刷;面料1印刷颜色:多色_CMYK;面料1覆膜:哑膜;面料2定型模切:面板;面料2材质:双铜纸_128;芯板纸板定型模切:纸板;芯板纸板材质:粉灰纸板_1200;内衬定型模切:面板;内衬材质:双铜纸_157;内衬印刷:普通印刷;内衬印刷颜色:多色_CMYK;内衬覆膜:哑膜;皮壳1尺寸:660_330;芯板纸板定型V槽:V槽_4;裱合工艺:裱纸;裱合工艺:贴片_铁片;|#|部件:内盒1;面料1定型模切:面板;面料1材质:双铜纸_157;面料1印刷:普通印刷;面料1印刷颜色:多色_CMYK;面料2定型模切:面板;面料2材质:粉灰纸_300;芯板纸板定型模切:纸板;打角:是;芯板纸板材质:粉灰纸板_600;内衬定型模切:面板;内衬材质:双铜纸_128;配件圆磁带铁片样式来源:傲彩标准;配件圆磁带铁片规格:12_16;内盒1尺寸:320_214_104;面料1包边:包到底;面料2包边:包到底;配件圆磁带铁片个数:1;裱合工艺:贴片_圆磁;;糊盒:机器;裱合工艺:裱纸;"
          let parentObj = _this.data2[0].attrList.find(e => e.miniZtAttrGroup.name == '部件');
          // let miaoliao = getobj.find(e => e.miniZtAttrGroup.name == '面料');
          // let caizhi = getobj.filter(e => e.miniZtAttrGroup.name == '材质' && e.miniZtAttrGroup.pname == '面料');
          console.log('材质', caizhi);
          let isOne = [];
          // let goodsAttr = `成品外尺寸:100_100_100;组装工艺:粘接;|#|部件:皮壳1;面料1定型模切:面板;面料1材质:双铜纸_128;皮壳1尺寸:400_100;|#|部件:面顶贴1;面料1定型模切:面板;面料1材质:白牛皮纸_120;面顶贴1尺寸:100_100;|#|部件:内盒1;面料1材质:PU皮_250;面料2定型模切:面板;面料2材质:银卡纸_215;内盒1尺寸:90_94_94;面料1包边:包到底;面料2包边:包到底;糊盒:机器;裱合工艺:裱纸;|#|部件:内围框1;面料1定型模切:面板;面料1材质:白牛皮纸_140;内围框1尺寸:348.9_98.36;|#|部件:皮壳2;面料1定型模切:面板;面料1材质:银卡纸_182;皮壳2尺寸:400_100;`;

          let arr = goodsAttr.split('|#|').map((e, index) =>
            e.split(';').map((s, index2) => {
              let ar = s.split(':');
              if (ar.length > 1) {
                if (ar[0] == '部件') {
                  isOne = /[\d]{1}/.exec(ar[1]);
                  if (_this.data2[0]) {
                    let attr = parentObj.attrList.find(e => ar[1].indexOf(e.name) != -1);
                    let ischeck = _this.parentdata.find(e => e == attr.id);
                    if (!ischeck) {
                      _this.parentchange(attr.id.toString(), attr.id, attr.name);
                      _this.parentdata.push(attr.id.toString());
                    } else if (ar[1] != attr.name + '1') {
                      // console.log('ararrrrrrrrrrrrrrrrrrrrr', ar[1]);
                      _this.addlay(attr.name);
                    }
                    // console.log('thisdcc', attr);
                    // console.log('csadascas', _this.parentdata);
                  }
                }
                // else if (/面料[\d]{1}材质/.test(ar[0])) {
                //   // if (ar[0] != '面料1材质') {
                //   //   console.log('(面料1材质？？？？？ ',ar)
                //   //   let citem = { attrList: miaoliao.attrList, miniZtAttrGroup: { ...miaoliao.miniZtAttrGroup, id: miaoliao.miniZtAttrGroup.id + 10000 + 15430 * index, groupName: index.toString() } };
                //   //   _this.addchild2(citem.miniZtAttrGroup.id + 12356, index, citem, 0);
                //   // }
                //   console.log('面料数据', ar, miaoliao, index);
                //   // if (miaoliao.miniZtAttrGroup.groupType == 'radio') {
                //   //   let valArr = ar[1].split('_');
                //   //   console.log('valArr', valArr);
                //   //   // let attr = miaoliao.attrList.find(e => e.name===valArr[0])
                //   //   let cz = null;
                //   //   let zzlx = caizhi.find(e => {
                //   //     cz = e.attrList.find(s => s.name == valArr[0]);
                //   //     return cz;
                //   //   });
                //   //   console.log('我的材质', zzlx);
                //   //   console.log('isOneisOneisOne', isOne);
                //   //   let idAdd = 0;
                //   //   if (isOne.length&&isOne[0] != '1') {
                //   //     idAdd = isOne[0] * 15430-15430;
                //   //     console.log('idAdd',idAdd)
                //   //   }
                //   //   let newId = zzlx.miniZtAttrGroup.pid+idAdd + 10000 + 15430 * index;
                //   //   let newId2 = zzlx.miniZtAttrGroup.parentAttrId+idAdd + 10000 + 15430 * index;
                //   //   _this.$set(_this.radiodata, newId, newId + '_' + newId2);
                //   //   // setTimeout(()=>{
                //   //   let citem = { attrList: miaoliao.attrList, miniZtAttrGroup: { ...miaoliao.miniZtAttrGroup, id: newId , groupName: index.toString() } };
                //   //   // _this.radiochange({ event: newId + '_' + newId2, child: citem.miniZtAttrGroup.actualValue, index: index, i: index2, name: citem.miniZtAttrGroup.name })
                //   //   // // },1000);
                //   //   radiochangeArr.push({ event: newId + '_' + newId2, child: citem.miniZtAttrGroup.actualValue, index: index, i: index2, name: citem.miniZtAttrGroup.name })
                //   //   console.log('_this.radiodata', _this.radiodata);
                //   //   console.log('新ID', newId, newId2);
                //   //   console.log('新IDeeeeeeeee', e);

                //   //   // _this.$set(_this.radiodata,newId,mycaizhi.miniZtAttrGroup.groupId+'_'+mycaizhi.miniZtAttrGroup.id)
                //   // }
                // }
              }
              return ar;
          ...
          // _this.radiochange({ event: '42792_80766', child: 'child', index: 1, i: 1, name: '面料'})
        }
        let goodsListArr = goodsAttr.split('|#|');
        let miaoliao = getobj.find(e => e.miniZtAttrGroup.name == '面料');
        let caizhi = getobj.filter(e => e.miniZtAttrGroup.name == '材质' && e.miniZtAttrGroup.pname == '面料');
        const test2 = () => {
          // console.log('_this.data2.', _this.data2.length);
          if (goodsListArr.length != _this.data2.length) {
            console.log('部件信息未加载完成~~~~');
            setTimeout(() => {
              test2();
            }, 500);
            return;
          }
          for (const index in _this.data2) {
            let item = _this.data2[index];
            console.log('item', item);
            if (index == 0) continue;
            let attrStr, attrArr;
            for (const index2 in _this.data2[index].attrList) {
              let citem = _this.data2[index].attrList[index2];
              // console.log('citem', citem);
              if (citem.miniZtAttrGroup.name === '部件') {
                attrStr = goodsListArr.find(e => e.indexOf(citem.attrList[0].actualValue) != -1);
                attrArr = attrStr.split(';');
                // console.log('ccc', attrStr);
              } else {
                if (attrArr) {
                  if (citem.miniZtAttrGroup.name == '尺寸') {
                    let reg = new RegExp(`${item.miniZtAttrGroup.name}[\\d]{1}${citem.miniZtAttrGroup.name}`);
                    let cc = attrArr.find(e => reg.test(e));
                    if (cc) {
                      let arr = cc.split(':')[1].split('_');
                      console.log('尺寸数组', arr);
                      for (let cindex = 0; cindex < citem.attrList.length; cindex++) {
                        let newId = citem.miniZtAttrGroup.id + '_' + citem.attrList[cindex].id;
                        _this.$set(_this.inputdata, newId, arr[cindex]);
                      }
                    }
                  } else if (citem.miniZtAttrGroup.name == '面料') {
                    let czArr = attrArr.filter(e => /面料[\d]{1}材质/.test(e));
                    if (czArr.length > 1) {
                      _this.addchild2(citem.miniZtAttrGroup.id + 12356, index, citem); //进行面料2的添加操作
                    }
                    // 遍历当前部件的面料材质数据字段，进行选中材质
                    for (let czIndex = 0; czIndex < czArr.length; czIndex++) {
                      let cz = null; //具体被选择材质（卡纸类---单铜纸）
                      //查找当前选中的纸张类型index（卡纸类/金银卡纸类）
                      let zzlxIndex = caizhi.findIndex(e => {
                        cz = e.attrList.find(s => czArr[czIndex].indexOf(s.name) != -1);
                        return cz;
                      });
                      let zzlx = caizhi[zzlxIndex]; //（卡纸类/金银卡纸类）
                      let newId = zzlx.miniZtAttrGroup.pid + 10000 + 15430 * index;
                      let newId2 = zzlx.miniZtAttrGroup.parentAttrId + 10000 + 15430 * index;
                      if (czIndex != 0) {
                        newId = zzlx.miniZtAttrGroup.pid + _this.copyp;
                        newId2 = zzlx.miniZtAttrGroup.parentAttrId + _this.copyp;
                      }
                      _this.$set(_this.radiodata, newId, newId + '_' + newId2);
                      _this.radiochange({ event: newId + '_' + newId2, child: citem.miniZtAttrGroup.actualValue, index: index, i: index2, name: citem.miniZtAttrGroup.name });
                      let czNewId = cz.groupId + 10000 + 15430 * index;
                      let czNewId2 = cz.id + 10000 + 15430 * index;
                      let zzIndex2 = index2 + 1;
                      if (czIndex != 0) {
                        czNewId = cz.groupId + _this.copyp;
                        czNewId2 = cz.id + _this.copyp;
                        zzIndex2 = zzIndex2 + (_this.copyp - 10000) / 15430;
                      }
                      _this.$set(_this.radiodata, czNewId, czNewId + '_' + czNewId2);
                      _this.radiochange({ event: czNewId + '_' + czNewId2, child: zzlx.miniZtAttrGroup.actualValue, index: index, i: zzIndex2, name: zzlx.miniZtAttrGroup.name });
                    }
                  } else {
                    console.log('数据数组', attrArr);
                    console.log('当前citem', citem.miniZtAttrGroup.name, citem);
        ...
        _this.$nextTick(() => {
          test();
          console.log('radiochangeArr', radiochangeArr);
          console.log(_this.data2);
          // _this.$nextTick(()=>{
          //   for (const item of radiochangeArr) {
          //     _this.radiochange(item)
          //   }
          // })
          _this.$nextTick(() => {
            test2();
        ...
        // console.log('thisdcc',this.data2)
        // setTimeout(()=>{
    ...
    addchild2(id, index, citem) {
      let _this = this;
      const num = citem.miniZtAttrGroup.groupName.split('_');
      const cnum = num[1];
      // if(!_this.data2[index]){
      //   setTimeout(()=>{
      //     _this.addchild2(id, index, citem)
      //   },500)
      //   return
      // }
      let mlength = this.data2[index].attrList.filter(v => {
