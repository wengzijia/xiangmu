      ...
      <el-col>
        <div class="guigeBox cardBox needhide">
          ...
          <div class="guigeLt">
              ...
              <el-form class="EattrMain attr-from" ref="formData" :model="formData" @submit.native.prevent size="small">
                <template v-for="(item, index) in data2">
                  <el-form-item
                    ...
                    v-if="item.miniZtAttrGroup.name != '部件信息' && item.miniZtAttrGroup.isPack == 1"
                  >
                    ...
                    <template v-for="(citem, i) in item.attrList">
                      <div v-if="citem.miniZtAttrGroup.isPack == 1" :key="i" class="laybox" v-show="citem.miniZtAttrGroup.actualValue != 'title'">
                        ...
                        <!-- {{i}} -->
                        <div v-if="citem.miniZtAttrGroup.showName == 0" v-show="showStartBorder(item.attrList, i)" class="laycon childactive" style="border-top:1px solid #eee;"></div>
                        <div :class="citem.miniZtAttrGroup.showName ? 'laycon' : 'laycon childactive'" v-if="citem.miniZtAttrGroup.reminder != 'hide'" :style="i == 0 ? 'border-top:1px solid #eee' : ''">
                          <div :class="!citem.miniZtAttrGroup.showName && citem.miniZtAttrGroup.code.indexOf('CHILD') != -1 ? 'laychild childactive3' : 'laychild'">
                            ...
                            <div v-if="citem.miniZtAttrGroup.code.indexOf('YSYS') > -1">
                            ...
                            <div v-if="citem.miniZtAttrGroup.code.indexOf('ZMYS') > -1">
                              <span class="yszs zmys" style="">正面专色数量</span>
                              <el-select
                                size="small"
                                v-model="spot[citem.miniZtAttrGroup.id]"
                                placeholder="请选择"
                        ...
                        <div class="laycon childactive" style="font-size: 12px;padding:0 0 0 3.6em;line-height: 16px;" v-if="citem.miniZtAttrGroup.name == '圆磁对数'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>1枚磁铁+1枚铁片组合为一对，磁铁安装在内盒，铁片安装在皮壳</div>
                          </div>
                        </div>
                        <!-- 包边 -->
                        <div class="laycon " style="font-size: 12px;margin-top: -5px;padding:0 0 0 3.8em;line-height: 16px;" v-if="citem.miniZtAttrGroup.name == '包边高度'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>包边高度应不小于20MM并且小于盒子高度，不录入默认包到底</div>
                          </div>
                        </div>
                        <!-- 书型盒内盒方磁提示 -->
                        <div class="laycon childactive" style="font-size: 12px;margin-top: -5px;padding:0 0 0 3.6em;line-height: 16px;" v-if="citem.miniZtAttrGroup.code == 'PJ_FC_FCDS_NH'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>2枚磁铁组合为一对，1枚安装在内盒，1枚安装在皮壳</div>
                          </div>
                        </div>
                        <!-- 平口天地盒天盖方磁提示 -->
                        <div class="laycon childactive" style="font-size: 12px;margin-top: -5px;padding:0 0 0 3.6em;line-height: 16px;" v-if="citem.miniZtAttrGroup.code == 'PJ_FC_FCDS_TG'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>2枚磁铁组合为一对，1枚安装在天盖，1枚安装在地盒</div>
                          </div>
                        </div>
                        <!-- 抽屉盒柜桶方磁提示 -->
                        <div class="laycon childactive" style="font-size: 12px;margin-top: -5px;padding:0 0 0 3.6em;line-height: 16px;" v-if="citem.miniZtAttrGroup.code == 'PJ_FC_FCDS_GT'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>2枚磁铁组合为一对，1枚安装在柜桶，1枚安装在抽屉</div>
                          </div>
                        </div>
                        <!-- 特种纸提示 -->
                        <div class="laycon childactive" style="font-size: 12px;padding:0 0 0 5.7em;line-height: 16px;" v-if="citem.miniZtAttrGroup.name == '材质' && citem.miniZtAttrGroup.parentAttrName == '特种纸'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>若须以您提供的颜色纹理为准，请先邮寄您的样品给傲彩，由傲彩产生物料编号供您下订单时录入。</div>
                          </div>
                        </div>
                        <!-- 灰底色板提示 -->
                        <div class="laycon childactive" style="font-size: 12px;padding:0 0 0 9em;line-height: 16px;" v-if="citem.miniZtAttrGroup.name.trim() == '' && citem.miniZtAttrGroup.parentAttrName == '灰底色板'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>若须以您提供的颜色纹理为准，请先邮寄您的样品给傲彩，由傲彩产生物料编号供您下订单时录入。</div>
                          </div>
                        </div>
                        <!-- 压纹提示 -->
                        <div class="laycon childactive" style="font-size: 12px;padding:0 0 0 5.7em;line-height: 16px;" v-if="citem.miniZtAttrGroup.name == '压纹'">
                          <div class="laychild">
                            <div><span class="red">*温馨提示：</span>若须以您提供的颜色纹理为准，请先邮寄您的样品给傲彩，由傲彩产生纹理编号供您下订单时录入。</div>
                          </div>
                        </div>
                        <!-- 配件方磁、圆磁以外提示 -->
                        <div class="laycon childactive" style="font-size: 12px;padding:0 0 0 3.6em;line-height: 16px;" v-if="citem.miniZtAttrGroup.name == '物料编号' && citem.miniZtAttrGroup.pname == '配件'">
                          <div class="laychild">
                      ...
                      <div v-if="showEndBorder(item.attrList, i, citem)" :key="i + '_border'" class="laycon childactive" style="border-bottom:1px solid #eee;padding:0;"></div>
                    </template>
            ...
            <div v-if="false" v-show="data2.length">
            ...
            <div class="laybox" v-if="false && group.indexOf('冲孔') > -1" style="padding-bottom:8px;">
              <div class="laycon childactive">
                <div class="laychild">
...
export default {
  ...
  methods: {
    ...
    showEndBorder(list, i, citem) {
      ...
      // let arr = list.map((citem, index, arr) => {
      //   let start = false;
      //   let end = false;
      //   if (index == 0) {
      //     start = true;
      //   } else if (arr[index - 1].miniZtAttrGroup.showName == 1 && arr[index].miniZtAttrGroup.isPack == 1) {
      //     start = true;
      //   }
      ...
      //   if (!item) {
      //     end = true;
      //   } else if (citem.miniZtAttrGroup.actualValue != 'title' && citem.miniZtAttrGroup.showName == 0 && item.miniZtAttrGroup.showName == 1 && arr[index].miniZtAttrGroup.isPack == 1) {
      //     end = true;
      //   }
      //   if (start != end) {
      //     if (!startIndex && start) {
      ...
      //   };
      // });
      // console.log('是否显示',arr)
      let item = list[i + 1];
      // console.log('item', item);
      if (!item) return true;
      return citem.miniZtAttrGroup.actualValue != 'title' && citem.miniZtAttrGroup.showName == 0 && item.miniZtAttrGroup.showName == 1 && citem.miniZtAttrGroup.isPack == 1;
    },
    ...{ strEquals },
    // 修改iframe
    getdata() {
      ...
      getVoList({
        ...
        console.log('newData2', _this.data.length);
        // this.setdata()
        let layid = [];
        ...
        console.log('旧数据', oldFormDataCopy);
                ...
                if (item.attrList.length) {
                  ...
                  if (item.attrList[0].miniZtAttrGroup) {
                    ...
                    for (let index2 = 0; index2 < data.attrList.length; index2++) {
                      let item2 = JSON.parse(JSON.stringify(data.attrList[index2]));
                      // 通过旧数据data2的id计算得出数据
                      ...
                      if (attr) {
                        let actualValue = item2.actualValue;
                        // 判断实际值是否一样
                        if (attr.actualValue !== item2.actualValue) {
                          ...
                          if (retArr && retArr.length && attr.actualValue) {
                            actualValue = item2.actualValue.replace(reg, retArr[0]);
                          } else {
                          ...
                          // console.log('面料数据不一致', attr.actualValue, item2.actualValue, actualValue);
                        }
                        newItem.attrList.push({ ...item2, id: attr.id, groupId: attr.groupId, actualValue });
                      }
                    }
                  }
                }
        ...
        const getNewData = function() {
          ...
          let getDataFunObj = {
            /**
             * 查找数据，把查找到的partlist和addarr这些数据合并到newData
             * @param {*} id 父级ID
             * @param {*} index
             * @param {*} num 面料1/2
             * @param {*} name 父级name
             * @param {*} copyp 用于生成新ID的copyp值
             */
            copypart(id, index, num = 1, name = '', copyp) {
              addarr = [];
              let partlist = [];
              if (name == '内围框') {
                partlist = [JSON.parse(JSON.stringify(newData2[0].attrList[1])), JSON.parse(JSON.stringify(newData2[0].attrList[2]))];
              } else if (num != 1 || name == '外连接' || name == '内连接') {
                partlist = [JSON.parse(JSON.stringify(newData2[0].attrList[1]))];
              } else {
                partlist = JSON.parse(JSON.stringify(newData2[0].attrList));
                partlist.shift();
              }
              console.log('partlist', partlist);
              for (let i = 0; i < partlist.length; i++) {
                partlist[i].miniZtAttrGroup.pid = id;
                partlist[i].miniZtAttrGroup.id = partlist[i].miniZtAttrGroup.id + copyp;
                partlist[i].miniZtAttrGroup.reminder = '';
                partlist[i].miniZtAttrGroup.pname = name;
                partlist[i].miniZtAttrGroup.sortOrder = partlist[i].miniZtAttrGroup.sortOrder + num * 1000;
                for (let j = 0; j < partlist[i].attrList.length; j++) {
                  getDataFunObj.lookupdata(partlist[i].attrList[j].groupId, partlist[i].attrList[j].id, name);
                  partlist[i].attrList[j].groupId = partlist[i].miniZtAttrGroup.id;
                  partlist[i].attrList[j].id = partlist[i].attrList[j].id + copyp;
                }
                if (partlist[i].miniZtAttrGroup.name == '面料' && num != 1) {
              ...
              for (let ii = 0; ii < addarr.length; ii++) {
                addarr[ii].miniZtAttrGroup.pid = addarr[ii].miniZtAttrGroup.pid + copyp;
                addarr[ii].miniZtAttrGroup.id = addarr[ii].miniZtAttrGroup.id + copyp;
                addarr[ii].miniZtAttrGroup.parentAttrId = addarr[ii].miniZtAttrGroup.parentAttrId + copyp;
                addarr[ii].miniZtAttrGroup.sortOrder = addarr[ii].miniZtAttrGroup.sortOrder + num * 1000;
                let actualValue = addarr[ii].miniZtAttrGroup.actualValue;
                if (num != 1 && actualValue) {
                  addarr[ii].miniZtAttrGroup.actualValue = actualValue.replace(/面料1/g, '面料' + num);
                }
                for (let jj = 0; jj < addarr[ii].attrList.length; jj++) {
                  getDataFunObj.lookupdata(addarr[ii].attrList[jj].groupId, addarr[ii].attrList[jj].id, name);
                  addarr[ii].attrList[jj].groupId = addarr[ii].miniZtAttrGroup.id;
                  addarr[ii].attrList[jj].id = addarr[ii].attrList[jj].id + copyp;
                  let actualValue = addarr[ii].attrList[jj].actualValue;
                  if (num != 1 && actualValue) {
                    // console.log('面料数据',actualValue)
                    addarr[ii].attrList[jj].actualValue = actualValue.replace(/面料1/g, '面料' + num);
                  }
              ...
              newData.push(...JSON.parse(JSON.stringify(partlist)));
              newData.push(...JSON.parse(JSON.stringify(addarr)));
            },

            /**
             * 检查需要更新的数据，把需要的需要加入addarr数组
             * @param {*} pid
             * @param {*} parentAttrId
             * @param {*} name
             */
            lookupdata(pid, parentAttrId, name) {
              ...
              let boxstr = '内盒,柜桶,抽屉,天盖,地盒';
              for (let i = 0; i < getobj.length; i++) {
                const groupItem = getobj[i];
                if (groupItem.miniZtAttrGroup.pid == pid && parentAttrId == groupItem.miniZtAttrGroup.parentAttrId) {
                  if (groupItem.attrList != null && groupItem.attrList.length > 0) {
                    if ((groupItem.miniZtAttrGroup.name == '是否裱盒内围' && boxstr.indexOf(name) == -1) || (groupItem.miniZtAttrGroup.name == '是否加海绵' && str.indexOf(name) == -1)) {
                      continue;
                    }
                    addarr.push(JSON.parse(JSON.stringify(groupItem)));
          ...
          for (let index = 0; index < newData2.length; index++) {
            ...
            if (index != 0) {
              // 查找面料的数据
              let arr = newData2[index].attrList.filter(e => e.miniZtAttrGroup.name === '面料');
              for (let i = 0; i < arr.length; i++) {
                // 在初始数组里面查询旧数据，然后根据新旧数据的差值获取到copyp
                let item = getobj.find(e => (arr[i].miniZtAttrGroup.id - e.miniZtAttrGroup.id - 10000) % 15430 == 0);
                let copyp = arr[i].miniZtAttrGroup.id - item.miniZtAttrGroup.id;
                copypArr.push(copyp);
                getDataFunObj.copypart(newData2[index].miniZtAttrGroup.id + 12345 + i, 1, i + 1, newData2[index].miniZtAttrGroup.name, copyp);
              }
            }
          }
          // 数据回显完需要改变copyp，不然新加的面料和部件回导致数据重复
          _this.copyp = Math.max(...copypArr);
        };
        getNewData2(oldData2, newData2);
        _this.$set(_this, 'data2', newData2);
        getNewData();
        _this.$set(_this, 'data', JSON.parse(JSON.stringify([].concat(res.data, newData))));
        _this.$set(_this, 'priceFormData', oldFormDataCopy.commonFormDatainputValue);
        _this.$set(_this, 'inputdata', oldFormDataCopy.formDatainputValue.inputdata); // 子集输入框
        _this.$set(_this, 'radiodata', oldFormDataCopy.formDatainputValue.radiodata);
        _this.$set(_this, 'checkdata', oldFormDataCopy.formDatainputValue.checkdata);
        _this.$set(_this, 'selectdata', oldFormDataCopy.formDatainputValue.selectdata);
        oldFormDataCopy.formDatainputValue.group && _this.$set(_this, 'group', oldFormDataCopy.formDatainputValue.group);
        oldFormDataCopy.formDatainputValue.hole && _this.$set(_this, 'hole', oldFormDataCopy.formDatainputValue.hole);
      });
    },
    initial() {
    ...
    getprice() {
      ...
      if (this.datastatus() && this.datastatus2() && this.priceFormData.long && this.priceFormData.wide && this.priceFormData.hign && this.priceFormData.num && this.fnumber > 0) {
        ...
        console.log('旧数据的goods', this.oldFormData);
