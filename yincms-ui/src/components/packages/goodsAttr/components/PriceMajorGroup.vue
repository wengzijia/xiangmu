<template>
  <div>
    <template v-for="(groupItem, index) in groupVoList">
      <div :key="'div_' + index" class="hardList" v-if="groupItem.isPack == 1">
        <h2 v-if="groupItem.name" class="flex hardH2">
          <span class="geH1Lt">{{ groupItem.name }}</span>
        </h2>
        <div style="color:red;margin: 10px 0 10px 64px;" v-if="cateId == '9' && groupItem.name == ''">说明书使用的是最广泛高松纸材质，与实际克重有差异，如有影响建议您选用更高克重材质</div>
        <price-major
      ...
      <div :key="'div_2' + index" class="hardList" style="padding: 15px 15px;" v-if="strEquals(groupItem.name, '热封底层信息') && strEquals(cateId, '13')">材质结构配比&厚度:{{ formData.zcjgpb }}</div>
    </template>
  </div>
</template>
...
import PriceMajor from './PriceMajor';
export default {
