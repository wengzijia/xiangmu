<template>
  <div class="EattrMbox packages-cart-box">
    <div class="guigeImg tgimg" v-if="tgimg" :style="baseIds.disposable ? 'width:200px' : ''">
      <img :src="tgimg" alt="" />
    </div>
    <el-form class="EattrMain attr-from" ref="formData" :model="formData" label-width="130px" @submit.native.prevent size="small">
      <template v-for="(item, index) in formVoList">
        ...
        <!-- <el-form-item :key="'reminderbkz_' + index" v-if="strEquals(baseIds.cateId, '2') && strEquals(item.miniZtAttrGroup.code, 'ZJGY')" label="见坑纸信息:">
          <template slot="label">
            <div class="EattrLab">
              <span class="red">*</span>
              见坑纸信息
        ...
        <!-- <el-form-item :key="'reminderwx_' + index" v-if="strEquals(baseIds.cateId, '70') && strEquals(item.miniZtAttrGroup.code, 'YSYS')">
          <template slot="label">
            <div class="EattrLab">
              <span class="red">*</span>
              材质信息
        ...
        <!--  -->
        <el-form-item :key="'reminder3_' + index" v-if="strEquals(baseIds.cateId, '2') && strEquals(item.miniZtAttrGroup.code, 'GY_TC')">
          <div class="Emtils">温馨提示：平台贴窗材质为PET，离折线≥15mm，用厚度为0.15±0.03mm胶片； 离折线＜12mm，用厚度为0.10±0.02mm胶片</div>
        </el-form-item>
        <el-form-item :key="'reminder2_' + index" v-if="strEquals(baseIds.cateId, '1') && strEquals(item.miniZtAttrGroup.code, 'ZJGY')">
        ...
        <el-form-item
          :key="index"
          v-if="strEquals(item.miniZtAttrGroup.isPack, '1')"
          :class="strEquals(item.miniZtAttrGroup.actualValue, 'isshow') ? 'ishow' : ''"
          :name="(baseIds.disposable && endWith(item.miniZtAttrGroup.code, 'HX')) || hidefu(item.miniZtAttrGroup.name, baseIds.subCateId, baseIds.cateId) ? 'hidename' : ''"
        >
          <!--工艺 弹层-->
          <template slot="label">
            <div class="EattrLab" :class="item.miniZtAttrGroup.actualValue == 'is' && item.miniZtAttrGroup.code.indexOf(msize) > 0 ? 'titlenone' : ''">
              <!-- :class="baseIds.cateId==122&&item.attrList[0].prefix.indexOf(msize)==-1&&item.attrList[0].prefix!='all'? 'titlenone1':''" -->
              <!-- {{msize}} -->

              <!--              不展示数据-->
              <template v-if="strEquals(baseIds.cateId, '86') || strEquals(baseIds.cateId, '121')">
              ...
              <template v-else>
                ...
                <!--                <span class="red" v-if="baseIds.cateId==10&&item.miniZtAttrGroup.name=='材质'">*</span>-->
                {{ item.miniZtAttrGroup.showName === 1 ? item.miniZtAttrGroup.name : '' }}
                <el-tooltip v-if="baseIds.cateId == 10 && endWith(item.miniZtAttrGroup.code, 'TGCC')" width="200" class="item" effect="light" placement="top-start">
                  <div slot="content">{{ item.miniZtAttrGroup.bindId == 94 ? '请输入直径，傲彩为您推荐尺寸相近铁罐' : '请输入长宽，傲彩为您推荐尺寸相近铁盒' }}</div>
                  <em class="guigeTis">?</em>
                </el-tooltip>
                <p style="line-height: 5px" v-if="baseIds.cateId == 10 && endWith(item.miniZtAttrGroup.code, 'TGCC')">{{ item.miniZtAttrGroup.bindId == 94 ? '(直径*高)' : '(长*宽*高)' }}</p>
              </template>
            </div>
          ...
          <el-row class="flex inputBx Layerinpts" style="flex-wrap:wrap" v-if="strEquals(item.miniZtAttrGroup.groupType, 'input')">
            ...
            <template v-if="endWith(item.miniZtAttrGroup.code, 'TGCC')">
              <div class="inputBxrow tgcc" v-for="attr in item.attrList" :key="attr.id">
                <el-input v-model="formData.inputValues[attr.groupId + '_' + attr.id]" :disabled="disabledflag" :placeholder="attr.inputTips" size="small" @focus="inpfocus" @input="tgchange(attr, attr.groupId + '_' + attr.id)">
                  <template slot="append">{{ attr.suffix }}</template>
                </el-input>
              </div>
              <span class="anew" v-if="anewflag" @click="anewfn">重新筛选</span>
            ...
            <template v-else>
              <div class="inputBxrow" v-for="attr in item.attrList" :key="attr.id">
                <el-input-number v-if="baseIds.subCateId == '120'" v-model="formData.inputValues[attr.groupId + '_' + attr.id]" :placeholder="attr.inputTips" size="small" @change="priceValuation" :min="0" :step="100" step-strictly>
                  <template slot="append">{{ attr.suffix }}</template>
                </el-input-number>
                <el-input
                  v-else
                  ...
                  size="small"
                  @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '', formData.inputValues[attr.groupId + '_' + attr.id], attr.suffix, attr.groupId + '_' + attr.id)"
                >
                  <template slot="append">{{ attr.suffix }}</template>
                </el-input>
              </div>
          ...
          <div class="flex" v-else-if="strEquals(item.miniZtAttrGroup.groupType, 'radio')" :class="item.miniZtAttrGroup.showName === 0 ? 'oAtrLayer' : ''">
            <!-- 塑料盒尺寸样式 :style="strEquals(baseIds.cateId, '86') && endWith(item.miniZtAttrGroup.code, 'CC') ? 'border: 1px solid #dbe0e4;padding: 20px 0 20px 20px;height:120px;overflow-y:scroll' : ''"-->
            <el-radio-group class="EattrMgroup" size="small" v-model="formData.radioSelectValues[item.miniZtAttrGroup.id]" @change="radioSelect" button-style="solid">
              <el-row class="Eattrrow" v-for="attr in item.attrList" :key="'attr_' + attr.id">
                <template v-if="attr.isPack === 1">
                  <template v-if="attr.remark != '' && attr.remark != null">
                    <!-- 后台访问不需要 显示图片说明 -->
                    <el-tooltip width="200" v-if="!baseIds.isCms" class="item" effect="light" placement="top-start" :enterable="baseIds.cateId == 86 || baseIds.cateId == 121 ? false : true">
                      <template slot="content">
                    ...
                    <el-radio-button :disabled="readioDisabled(item)" size="small" v-else :label="attr.groupId + '_' + attr.id">{{ attr.name }}</el-radio-button>
                  </template>
                  <!--                不展示数据-->
                  <template v-else-if="(strEquals(baseIds.cateId, '86') || strEquals(baseIds.cateId, '121')) && endWith(item.miniZtAttrGroup.code, 'SL')"></template>
                  <template v-else>
                    <template>
                      ...
                      <template v-else-if="baseIds.cateId == 122">
                      ...
                      <el-radio-button size="small" :disabled="item.miniZtAttrGroup.name == '品类' && baseIds.cateId == 13 ? (baseIds.flat == 'true' ? true : false) : readioDisabled(item, attr)" :label="attr.groupId + '_' + attr.id" v-else>
                        {{ attr.name }}
                        <!-- {{attr.id}}
                      {{attr.groupId}} -->
                      </el-radio-button>
            ...
            <!-- 专色 -->
            <span class="colorNumbr" v-if="(strEquals(baseIds.cateId, '8') || strEquals(baseIds.subCateId, '40')) && strEquals(item.miniZtAttrGroup.code, 'YSYS')">
            ...
            <!-- 卡纸盒自定义盒型粘合显示 -->
            <span v-if="item.miniZtAttrGroup.name == '粘合' && strEquals(baseIds.subCateId, '24')">(此粘合是指粘口位的粘合工艺)</span>
            <span v-if="strEquals(bindType, 'cate')">
              ...
              <span class="colorNumbr" v-if="endWith(item.miniZtAttrGroup.code, 'YSXZ')">
                <el-checkbox-button @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" :disabled="noFlatDisabled" class="major-check" v-if="formData.dmys.jybm" v-model="formData.dmys.jybmVal" label="加印白墨"
                  >加印白墨</el-checkbox-button
                >
                专色数量:
                <el-select v-model="formData.dmys.scCount" style="width: 100px" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" :disabled="noFlatDisabled">
                  <el-option label="0" :value="0"></el-option>
                  <el-option label="1" :value="1"></el-option>
                  <el-option label="2" :value="2"></el-option>
              ...
              <span class="colorNumbr" v-if="endWith(item.miniZtAttrGroup.code, 'BK_YSYS')">
                专色数量:
                <el-select v-model="formData.bqys.scCount" style="width: 100px" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" :disabled="noFlatDisabled">
                  <el-option label="0" :value="0"></el-option>
                  <el-option label="1" :value="1"></el-option>
                  <el-option label="2" :value="2"></el-option>
                  <el-option label="3" :value="3"></el-option>
              ...
              <span
                class="colorNumbr"
                v-if="
                  ((strEquals(baseIds.cateId, '2') && item.miniZtAttrGroup.parentAttrName != '裱卡') ||
                    (strEquals(baseIds.cateId, '4') && strEquals(baseIds.subCateId, '35') && item.miniZtAttrGroup.code == 'YSYS') ||
                    (strEquals(baseIds.cateId, '9') && strEquals(baseIds.subCateId, '46') && formData.radioSelectValues['6668'] && formData.radioSelectValues['6668'].indexOf('6668_29877') > -1) ||
                    (strEquals(baseIds.cateId, '81') && strEquals(baseIds.subCateId, '82')) ||
                    strEquals(baseIds.cateId, '70') ||
                    strEquals(baseIds.cateId, '13')) &&
              ...
              >
                <el-checkbox-button
                  ...
                  class="major-check"
                  :disabled="noFlatDisabled"
                  v-if="formData.dmys.jybm && !(strEquals(baseIds.cateId, '9') && strEquals(baseIds.subCateId, '46'))"
                  v-model="formData.dmys.jybmVal"
                  label="加印白墨"
                  >加印白墨</el-checkbox-button
                >
                专色数量:
                <el-select v-if="strEquals(baseIds.cateId, '81') && strEquals(baseIds.subCateId, '82')" :disabled="noFlatDisabled" v-model="formData.dmys.scCount" style="width: 70px" @change="priceValuation()">
                  <el-option label="0" :value="0"></el-option>
                  <el-option label="1" :value="1"></el-option>
                  <el-option label="2" :value="2"></el-option>
                  <el-option label="3" :value="3"></el-option>
                ...
                <el-select v-else v-model="formData.dmys.scCount" style="width: 70px" @change="priceValuation" :disabled="noFlatDisabled">
                  <el-option label="0" :value="0"></el-option>
                  <el-option label="1" :value="1"></el-option>
                  <el-option label="2" :value="2"></el-option>
                  <el-option label="3" :value="3"></el-option>
                  <el-option v-if="baseIds.cateId !== '2'" label="4" :value="4"></el-option>
                </el-select>
              </span>
              <span class="colorNumbr" v-if="strEquals(baseIds.cateId, '10') && endWith(item.miniZtAttrGroup.code, 'YSYS')">
              ...
              <span class="colorNumbr" v-if="endWith(item.miniZtAttrGroup.code, 'SMYS_ZM')">
                <el-checkbox-button @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" class="major-check" v-if="formData.zmys.jybm" v-model="formData.zmys.jybmVal" label="加印白墨">加印白墨</el-checkbox-button>
                专色数量:
                <el-select v-model="formData.zmys.scCount" style="width: 100px" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" :disabled="noFlatDisabled">
                  <el-option label="0" :value="0"></el-option>
                  <el-option label="1" :value="1"></el-option>
                  <el-option label="2" :value="2"></el-option>
                  <el-option label="3" :value="3"></el-option>
              ...
              <span class="colorNumbr" v-if="endWith(item.miniZtAttrGroup.code, 'SMYS_FM')">
                <el-checkbox-button @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" class="major-check" v-if="formData.fmys.jybm" v-model="formData.fmys.jybmVal" label="加印白墨">加印白墨</el-checkbox-button>
                专色数量:
                <el-select v-model="formData.fmys.scCount" style="width: 100px" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" :disabled="noFlatDisabled">
                  <el-option label="0" :value="0"></el-option>
                  <el-option label="1" :value="1"></el-option>
                  <el-option label="2" :value="2"></el-option>
                  <el-option label="3" :value="3"></el-option>
              ...
              <span class="colorNumbr" v-if="strEquals(baseIds.cateId, '70') && endWith(item.miniZtAttrGroup.code, 'YSYS')">
                印刷面:
                <el-select v-model="formData.ysm.scCount" style="width: 70px" @change="ysmChange" :disabled="noFlatDisabled">
                  <el-option label="0" :value="0"></el-option>
                  <el-option label="1" :value="1"></el-option>
                  <el-option label="2" :value="2"></el-option>
                  <el-option label="3" :value="3"></el-option>
            ...
            <el-checkbox-group class="EattrMgroup" size="small" v-model="formData.checkBoxSelectValues[item.miniZtAttrGroup.id]" :disabled="noFlatDisabled" @change="checkboxSelect({ event: $event, code: item.miniZtAttrGroup.id })">
              <el-row class="Eattrrow" v-for="attr in item.attrList" :key="'attr_' + attr.id">
                <template v-if="attr.isPack == 1">
                  ...
                  <el-checkbox-button size="small" v-show="attr.prefix.indexOf(msize) > -1 || attr.prefix == 'all'" :class="attr.defaultValue == 'img' ? 'padd' : ''" :label="attr.groupId + '_' + attr.id" v-else-if="baseIds.cateId == 122">
                    <template v-if="attr.defaultValue == 'img'"><img width="50px" :src="attr.name"/></template>
                    <template v-else>{{ attr.name }}</template>
                    <!-- {{attr.prefix}} -->
                  </el-checkbox-button>
                  <el-checkbox-button v-else :label="attr.groupId + '_' + attr.id" :class="baseIds.subCateId == 28 && attr.id == '53504' && formData.radioSelectValues[5896] == '5896_32267' ? 'hideclass' : ''" :disabled="readioDisabled(item)">{{
                    attr.name
                  }}</el-checkbox-button>
                </template>
              </el-row>
            </el-checkbox-group>
            <span class="colorNumbr" v-if="strEquals(baseIds.cateId, '10') && item.miniZtAttrGroup.code == 'YSGY'">
              <!-- <el-checkbox-button @change="priceValuation" class="major-check" v-if="formData.dmys.jybm" v-model="formData.dmys.jybmVal" label="加印白墨">加印白墨</el-checkbox-button> -->
              专色数量:
          ...
          <div v-else-if="strEquals(item.miniZtAttrGroup.groupType, 'select')">
            <el-select size="small" v-model="formData.selectSelectValues[item.miniZtAttrGroup.id]" placeholder="请选择" :disabled="noFlatDisabled" @change="selectSelect">
              <template v-for="(attr, aindex) in item.attrList">
                <el-option v-if="attr.isPack == 1" :key="aindex" :label="attr.name" :value="attr.groupId + '_' + attr.id"> </el-option>
              </template>
            </el-select>
            <!-- <template v-if="baseIds.cateId==10&&baseIds.subCateId==95">
          ...
          <div v-if="strEquals(item.miniZtAttrGroup.code, 'BMCL_BRB_BRCS')">
            <span class="velvetTimesTips f12 C9">默认同一产品多次裱绒为同一颜色,如有不同请联系<span class="red" style="cursor: pointer;" @click="qiyu">在线客服</span>进行备注,给您造成不便敬请谅解</span>
          </div>
          <!-- 潘通色号 -->
          ...
          <div class="PantoneDv" v-if="endWith(item.miniZtAttrGroup.code, 'YSXZ') && formData.dmys.scCount > 0">
            <label>请填写PANTONE色号:</label>
            <el-input class="PantoneDvtx" v-for="(item, index) in formData.dmys.scCount" :key="item" v-model="formData.dmys.pantoneList[index]" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" />
            <p class="toolbox">
              <span class="red">*</span>官方默认以潘通色卡版本“
              <el-tooltip width="200" class="item" effect="light" placement="top-start" :enterable="false">
                <template slot="content">
          ...
          <div
            class="PantoneDv"
            v-if="
              (strEquals(baseIds.cateId, '2') ||
                (strEquals(baseIds.cateId, '4') && strEquals(baseIds.subCateId, '35') && item.miniZtAttrGroup.code == 'YSYS') ||
                (strEquals(baseIds.cateId, '9') && strEquals(baseIds.subCateId, '46') && formData.radioSelectValues['6668'] && formData.radioSelectValues['6668'].indexOf('6668_29877') > -1) ||
                strEquals(baseIds.cateId, '10') ||
                (strEquals(baseIds.cateId, '81') && strEquals(baseIds.subCateId, '82')) ||
                strEquals(baseIds.cateId, '70') ||
                strEquals(baseIds.cateId, '13')) &&
                endWith(item.miniZtAttrGroup.code, 'YSYS') &&
          ...
          <div class="PantoneDv" v-if="endWith(item.miniZtAttrGroup.code, 'SMYS_ZM') && formData.zmys.scCount > 0">
            <label>请填写PANTONE色号:</label>
            <el-input class="PantoneDvtx" v-for="(item, index) in formData.zmys.scCount" :key="item" v-model="formData.zmys.pantoneList[index]" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" />
            <p class="toolbox">
              <span class="red">*</span>官方默认以潘通色卡版本“
              <el-tooltip width="200" class="item" effect="light" placement="top-start" :enterable="false">
                <template slot="content">
          ...
          <div class="PantoneDv" v-if="endWith(item.miniZtAttrGroup.code, 'SMYS_FM') && formData.fmys.scCount > 0">
            <label>请填写PANTONE色号:</label>
            <el-input class="PantoneDvtx" v-for="(item, index) in formData.fmys.scCount" :key="item" v-model="formData.fmys.pantoneList[index]" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" />
            <p class="toolbox">
              <span class="red">*</span>官方默认以潘通色卡版本“
              <el-tooltip width="200" class="item" effect="light" placement="top-start" :enterable="false">
                <template slot="content">
          ...
          <div class="PantoneDv" v-if="endWith(item.miniZtAttrGroup.code, 'BJYGY_BK_YSYS1') && formData.bj1ys.scCount > 0">
            <label>请填写PANTONE色号:</label>
            <el-input class="PantoneDvtx" v-for="(item, index) in formData.bj1ys.scCount" :key="item" v-model="formData.bj1ys.pantoneList[index]" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" />
            <p class="toolbox">
              <span class="red">*</span>官方默认以潘通色卡版本“
              <el-tooltip width="200" class="item" effect="light" placement="top-start" :enterable="false">
                <template slot="content">
          ...
          <div class="PantoneDv" v-if="endWith(item.miniZtAttrGroup.code, 'BJEGY_BK_YSYS2') && formData.bj2ys.scCount > 0">
            <label>请填写PANTONE色号:</label>
            <el-input class="PantoneDvtx" v-for="(item, index) in formData.bj2ys.scCount" :key="item" v-model="formData.bj2ys.pantoneList[index]" @change="priceValuation(baseIds.cateId == 1 ? getraradiofn() : '')" />
            <p class="toolbox">
              <span class="red">*</span>官方默认以潘通色卡版本“
              <el-tooltip width="200" class="item" effect="light" placement="top-start" :enterable="false">
                <template slot="content">
          ...
          <div class="PantoneDv" v-if="strEquals(baseIds.cateId, '70') && endWith(item.miniZtAttrGroup.code, 'YSYS')">
            <ul>
              <li class="flex inputBx onetx" v-for="(item, index) in formData.ysm.scCount" :key="index">
                <label>实际印刷尺寸{{ index + 1 }}</label>
                <el-input class="PantoneDvtx noMrgin" v-model="formData.ysm.list[index]['longVal']" @change="priceValuation"> <template slot="append" s>x</template>实际印刷尺寸 </el-input>
          ...
          <slot name="zzzl" v-if="item.miniZtAttrGroup.code === 'ZZCZ_KZL_ZZKZHD'"></slot>
        </el-form-item>

        <el-form-item :key="'reminderwx_' + index" v-if="index == formVoList.length - 1 && (baseIds.subCateId === '85' || baseIds.subCateId === '96')">
          <template slot="label">
            <div class="EattrLab">
              <span class="red">*</span>
              材质信息
      ...
      <!-- <el-form-item v-if="baseIds.cateId == 1 || baseIds.cateId == 2"> -->
...
import { isCopyCraft } from '../price/craftUtil';
import { reSetSize, cateChange } from '../price/goodPriceUtil';
...
export default {
  ...
  methods: {
    ...
    hidefu(name, id, cateId) {
      let flag = true;
      if (name == '印刷类型' && (id == '29' || id == '30' || id == '31')) {
        flag = true;
      } else {
        flag = false;
      }
    ...
    clickitem(e, n, nid) {
      if (this.baseIds.isCms === 'edit' && this.baseIds.flat === 'false') {
        return;
      }
      let flagdel = null;
    ...
    readioDisabled(item = {}, attr = {}) {
      if (this.baseIds.subCateId == 42 && this.formData.checkBoxSelectValues[15299]?.indexOf('15299_48591') > -1 && attr.id == 48865) {
        return true;
      }
      // 是否为后台访问
      if (!this.baseIds.isCms) {
        return false;
      }
      // 新增 并且非平面
      if (this.baseIds.isCms === 'add' && this.baseIds.flat === 'false') {
        return false;
      }
      let disabled = false;
      // 新增/编辑 并且是平面
  ...
  watch: {
    ...
    // $route: {
    //   handler() {
    //     if (this.baseIds.subCateId == 93 || this.baseIds.subCateId == 94 || this.baseIds.subCateId == 95) {
    //       // console.log(111)
    //       this.cclist = [];
    //       this.ccdata = '';
    //       this.anewflag = false;
