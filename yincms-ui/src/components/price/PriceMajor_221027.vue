        ...
        >
          ...
          <el-row class="flex inputBx Layerinpts" style="flex-wrap:wrap" v-if="strEquals(item.miniZtAttrGroup.groupType, 'input')">
            ...
            <template v-if="endWith(item.miniZtAttrGroup.code, 'TGCC')">
              ...
              <!-- 后台访问 -->
              <span class="anew" v-if="$route.query.isCms && noFlatDisabled" @click="anewfn">重新筛选</span>
          ...
          <div v-else-if="strEquals(item.miniZtAttrGroup.groupType, 'checkbox')">
            ...
            <el-checkbox-group class="EattrMgroup" size="small" v-model="formData.checkBoxSelectValues[item.miniZtAttrGroup.id]" :disabled="noFlatDisabled" @change="checkboxSelect({ event: $event, code: item.miniZtAttrGroup.id })">
              <el-row class="Eattrrow" v-for="attr in item.attrList" :key="'attr_' + attr.id">
                ...
                <el-checkbox-button v-else :label="attr.groupId + '_' + attr.id" :class="$route.query.subCateId == 28 && attr.id == '53504' && formData.radioSelectValues[5896] == '5896_32267' ? 'hideclass' : ''" :disabled="readioDisabled(item)">{{
                  attr.name
                }}</el-checkbox-button>
              </el-row>
            </el-checkbox-group>
          ...
          <div v-if="strEquals(item.miniZtAttrGroup.code, 'BMCL_BRB_BRCS')">
            <span class="velvetTimesTips f12 C9"
              >默认同一产品多次裱绒为同一颜色,如有不同请联系<a href="http://wpa.b.qq.com/cgi/wpa.php?ln=1&amp;key=XzgwMDgyMjAwNl80NzE5OTVfODAwODIyMDA2XzJf" class="red" target="_blank">在线客服</a>进行备注,给您造成不便敬请谅解</span
            >
          </div>
          <!-- 潘通色号 -->
...
import { reSetSize, cateChange } from '@/utils/price/goodPriceUtil';
...
export default {
  ...
  methods: {
    ...
    readioDisabled(item = {}, attr = {}) {
      if (this.$route.query.subCateId == 42 && this.formData.checkBoxSelectValues[15299].indexOf('15299_48591') > -1 && attr.id == 48865) {
        return true;
      }
      // 是否为后台访问
