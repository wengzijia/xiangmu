            ...
            <div class="box">
              <el-carousel direction="vertical" :autoplay="isAutoplay" :interval="5000" :loop="true">
                <el-carousel-item v-for="(item, index) in contentList" :key="index">
                  <li class="flex noticeDv" style="color: #000;font-size: 16px;cursor: pointer;" @click="newsDetail(item.id)" title="点击查看公告详情">
    ...
    <div v-if="false">
      <el-dialog :visible.sync="dialogVisible" width="30%" :append-to-body="true" :modal="true" top="270px" :show-close="false" custom-class="dialog-activityimg">
        <div @click="newsDetails">
          <img src="../../static/mulindex/img/index/activityimg.png" style="height: 400px;width: 100%;display: block" />
          <!--          <img @click="newsDetails" src="@/static/index/img/quotation.png" style="height: 400px;width: 100%;display: block" />-->
          <span @click.stop="deleteBun" style="position: absolute;top: 0;right: 1px;color: #9a9a9a;padding: 20px;cursor: pointer">{{ nub }}s关闭</span>
        </div>
      </el-dialog>
    </div>
...
<script>
import { getById, listByIds, getNotice } from '@/api/news/newsApi';
...
export default {
  ...
  watch: {
    '$store.getters.noticeList': function(val) {
      let _this = this;
      let res = {
        data: val
      };
      if (res.data.length > 1) {
        _this.contentList = res.data.concat(res.data);
        _this.isAutoplay = true;
      } else {
        if (res.data.length > 0) {
          _this.contentList = [].concat(res.data);
        }
      }
    }
  },
  methods: {
    getDesc() {
      let _this = this;
      if (this.$store.getters.noticeList.length == 0) {
        _this.$store.dispatch('getTopNotice');
      } else {
        _this.contentList = _this.$store.getters.noticeList;
        _this.isAutoplay = true;
      }
    },
    activitesDeleteBut() {
  ...
  created() {
    ...
    //活动 6.19 - 7.18
    if ((dates >= 19 && month == 5) || (dates <= 18 && month == 6)) {
      //判断是否已显示过
