...
Page({
  ...
  async getCurrentInfo() {
    ...
    let {
      ...
      data,
      message
    } = await getCurrentOrderInfo(deviceId);
    console.log('当前订单数据', data);
    if (code == '000000') {
      if (data) {
  ...
  previewImage(e) {
    ...
    // if(Object.prototype.toString.call(imglist))
    if (imglist.length) {
  ...
  async donePro(e) {
    ...
    if (e.currentTarget.dataset) {
      ...
      if (code == '000000') {
        wx.navigateTo({
          url: `/subpackageA/pages/donePromote/index?groupNo=${groupno}&mainId=${mainid}&poFinishList=${polist}&nearCount=${nearcount}`
        })
      } else if (code == '910024') {
        _this.setData({
  ...
  async jumpReported() {
    ...
    if (code == '000000') {
      wx.navigateTo({
        url: `/subpackageA/pages/reported/index?attributeId=${attributeId}&groupId=${groupId}&groupSn=${groupSn}&poSns=${poSns}&mainId=${mainId}`
      })
    } else if (code == '980001' || code == '910023') {
      _this.setData({
        isShowHint: true
  ...
  onShow: function () {
    ...
    console.log('是否登录', _this.data.isLogin);
